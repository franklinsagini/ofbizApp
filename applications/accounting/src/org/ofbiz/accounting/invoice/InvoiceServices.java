 ...
 * 
 * Critically important: BigDecimal arithmetic methods like add(), multiply(),
 * divide() do not modify the BigDecimal itself. Instead, they return a new
 * BigDecimal. For example, to keep a running total of an amount, make sure you
 * do this:
 * 
 ...
 * 
 * Use .setScale(scale, roundingMode) after every computation to scale and round
 * off the decimals. Check the code to see how the scale and roundingMode are
 * obtained and how the function is used.
 * 
 * use .compareTo() to compare big decimals
 * 
 * ex. (amountOne.compareTo(amountTwo) == 1) checks if amountOne is greater than
 * amountTwo
 * 
 * Use .signum() to test if value is negative, zero, or positive
 * 
 * ex. (amountOne.signum() == 1) checks if the amount is a positive non-zero
 * number
 * 
 * Never use the .equals() function becaues it considers 2.0 not equal to 2.00
 * (the scale is different) Instead, use .compareTo() or .signum(), which
...
public class InvoiceServices {
	...
	private static final int INVOICE_ITEM_SEQUENCE_ID_DIGITS = 5; // this is the
																	// number of
																	// digits
																	// used for
																	// invoiceItemSeqId:
	...
	public static Map<String, Object> createInvoiceForOrderAllItems(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			List<GenericValue> orderItems = delegator.findByAnd("OrderItem",
					UtilMisc.toMap("orderId", (String) context.get("orderId")), UtilMisc.toList("orderItemSeqId"),
					false);
			if (orderItems.size() > 0) {
				context.put("billItems", orderItems);
			}
			// get the system userid and store in context otherwise the invoice
			// add service does not work
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(resource,
					"AccountingEntityDataProblemCreatingInvoiceFromOrderItems", UtilMisc.toMap("reason", e.toString()),
					locale));
	...
	public static Map<String, Object> createInvoiceForOrder(DispatchContext dctx, Map<String, Object> context) {
		...
		if (UtilValidate.isEmpty(billItems)) {
			...
			return ServiceUtil.returnSuccess(UtilProperties.getMessage(resource, "AccountingNoOrderItemsToInvoice",
					locale));
		}
		...
		try {
			...
			// get the shipping adjustment mode (Y = Pro-Rate; N =
			// First-Invoice)
			...
			// these variables are for pro-rating order amounts across invoices,
			// so they should not be rounded off for maximum accuracy
			...
			if (UtilValidate.isEmpty(invoiceDate)) {
				// TODO: ideally this should be the same time as when a shipment
				// is sent and be passed in as a parameter
			...
			// TODO: it might be nice to filter OrderTerms to only copy over
			// financial terms.
			...
			if (billingAccount != null) {
				/*
				 * jacopoc: billing account terms were already copied as order
				 * terms when the order was created. // get the billing account
				 * terms billingAccountTerms =
				 * billingAccount.getRelated("BillingAccountTerm", null, null,
				 * false);
				 * 
				 * // set the invoice terms as defined for the billing account
				 * createInvoiceTerms(delegator, dispatcher, invoiceId,
				 * billingAccountTerms, userLogin, locale);
				 */
				...
				for (GenericValue billToRole : billToRoles) {
					if (!(billToRole.getString("partyId").equals(billToCustomerPartyId))) {
						createInvoiceRoleContext = UtilMisc.toMap("invoiceId", invoiceId, "partyId",
								billToRole.get("partyId"), "roleTypeId", "BILL_TO_CUSTOMER", "userLogin", userLogin);
						Map<String, Object> createInvoiceRoleResult = dispatcher.runSync("createInvoiceRole",
								createInvoiceRoleContext);
						if (ServiceUtil.isError(createInvoiceRoleResult)) {
							return ServiceUtil.returnError(UtilProperties.getMessage(resource,
									"AccountingErrorCreatingInvoiceRoleFromOrder", locale), null, null,
									createInvoiceRoleResult);
						}
					}
				}

				// set the bill-to contact mech as the contact mech of the
				// billing account
				if (UtilValidate.isNotEmpty(billingAccount.getString("contactMechId"))) {
					...
					if (ServiceUtil.isError(createBillToContactMechResult)) {
						return ServiceUtil.returnError(UtilProperties.getMessage(resource,
								"AccountingErrorCreatingInvoiceContactMechFromOrder", locale), null, null,
								createBillToContactMechResult);
					}
				}
			} else {
				...
				if (UtilValidate.isNotEmpty(billingLocations)) {
					for (GenericValue ocm : billingLocations) {
						Map<String, Object> createBillToContactMechContext = UtilMisc.toMap("invoiceId", invoiceId,
								...
								"BILLING_LOCATION", "userLogin", userLogin);
						Map<String, Object> createBillToContactMechResult = dispatcher.runSync(
								"createInvoiceContactMech", createBillToContactMechContext);
						if (ServiceUtil.isError(createBillToContactMechResult)) {
							return ServiceUtil.returnError(UtilProperties.getMessage(resource,
									"AccountingErrorCreatingInvoiceContactMechFromOrder", locale), null, null,
									createBillToContactMechResult);
			...
			// DEJ20050705 doesn't appear to be used: List paymentPreferences =
			// orderHeader.getRelated("OrderPaymentPreference", null, null,
			// false);
			...
			if (invoiceType.equals("PURCHASE_INVOICE")) {
				...
				if (billFromVendor != null) {
					List<GenericValue> billingContactMechs = billFromVendor.getRelatedOne("Party", false).getRelated(
							"PartyContactMechPurpose", UtilMisc.toMap("contactMechPurposeTypeId", "BILLING_LOCATION"),
							null, false);
				...
				// for sales orders, it is the payment address on file for the
				// store
			...
			if (payToAddress != null) {
				...
				if (ServiceUtil.isError(createPayToContactMechResult)) {
					return ServiceUtil.returnError(UtilProperties.getMessage(resource,
							"AccountingErrorCreatingInvoiceContactMechFromOrder", locale), null, null,
							createPayToContactMechResult);
				}
			}

			// sequence for items - all OrderItems or InventoryReservations +
			// all Adjustments
			...
			for (GenericValue currentValue : billItems) {
				...
				createInvoiceItemContext.put(
						"invoiceItemTypeId",
						getInvoiceItemType(delegator, (orderItem.getString("orderItemTypeId")), (product == null ? null
				...
				if (ServiceUtil.isError(createInvoiceItemResult)) {
					return ServiceUtil.returnError(
							UtilProperties.getMessage(resource, "AccountingErrorCreatingInvoiceItemFromOrder", locale),
				...
				if (shippingApplies) {
					invoiceShipProRateAmount = invoiceShipProRateAmount.add(thisAmount).setScale(invoiceTypeDecimals,
							ROUNDING);
				}
				...
				if (ServiceUtil.isError(createOrderItemBillingResult)) {
					return ServiceUtil.returnError(UtilProperties.getMessage(resource,
							"AccountingErrorCreatingOrderItemBillingFromOrder", locale), null, null,
							createOrderItemBillingResult);
				}

				if ("ItemIssuance".equals(currentValue.getEntityName())) {
					List<GenericValue> shipmentItemBillings = delegator.findByAnd("ShipmentItemBilling", UtilMisc
							.toMap("shipmentId", currentValue.get("shipmentId"), "shipmentItemSeqId",
									currentValue.get("shipmentItemSeqId")), null, false);
				...
				// Get the original order item from the DB, in case the quantity
				// has been overridden
				GenericValue originalOrderItem = delegator.findOne("OrderItem",
				...
				for (GenericValue adj : itemAdjustments) {
					...
					// adjustment amount, the full amount has already been
					// invoiced,
					// so skip this adjustment
					if (adj.get("amount") == null) { // JLR 17/4/7 : fix a bug
														// coming from POS in
														// case of use of a
														// discount (on item(s)
														// or sale, item(s)
					...
					if (amount.signum() != 0) {
						...
						createInvoiceItemAdjContext.put(
								"invoiceItemTypeId",
						...
						String description = (UtilValidate.isEmpty(adj.getString("description")) ? adj
								.getString("comments") : adj.getString("description"));
						createInvoiceItemAdjContext.put("description", description);

						// invoice items for sales tax are not taxable
						// themselves
						// TODO: This is not an ideal solution. Instead, we need
						// to use OrderAdjustment.includeInTax when it is
						// implemented
						if (!(adj.getString("orderAdjustmentTypeId").equals("SALES_TAX"))) {
							createInvoiceItemAdjContext.put("taxableFlag", product.get("taxable"));
						}

						// If the OrderAdjustment is associated to a
						// ProductPromo,
						// and the field ProductPromo.overrideOrgPartyId is set,
						// copy the value to InvoiceItem.overrideOrgPartyId:
						// this
						// represent an organization override for the
						// payToPartyId
						if (UtilValidate.isNotEmpty(adj.getString("productPromoId"))) {
							try {
								...
								Debug.logError(e,
										"Error looking up ProductPromo with id [" + adj.getString("productPromoId")
												+ "]", module);
							}
						}
						...
						if (ServiceUtil.isError(createInvoiceItemAdjResult)) {
							return ServiceUtil.returnError(UtilProperties.getMessage(resource,
									"AccountingErrorCreatingInvoiceItemFromOrder", locale), null, null,
									createInvoiceItemAdjResult);
						}
						...
						createOrderAdjustmentBillingContext
								.put("orderAdjustmentId", adj.getString("orderAdjustmentId"));
						...
						Map<String, Object> createOrderAdjustmentBillingResult = dispatcher.runSync(
								"createOrderAdjustmentBilling", createOrderAdjustmentBillingContext);
						...
						// adjustments only apply to totals when they are not
						// tax or shipping adjustments
						if (!"SALES_TAX".equals(adj.getString("orderAdjustmentTypeId"))
							...
							// add to the ship amount only if it applies to this
							// item
							if (shippingApplies) {
								invoiceShipProRateAmount = invoiceShipProRateAmount.add(thisAdjAmount).setScale(
										invoiceTypeDecimals, ROUNDING);
							}
						}
			...
			// create header adjustments as line items -- always to tax/shipping
			// last
			...
			for (GenericValue adj : headerAdjustments) {
				...
				// Check against OrderAdjustmentBilling to see how much of this
				// adjustment has already been invoiced
				BigDecimal adjAlreadyInvoicedAmount = null;
				try {
					Map<String, Object> checkResult = dispatcher.runSync("calculateInvoicedAdjustmentTotal",
							UtilMisc.toMap("orderAdjustment", adj));
					adjAlreadyInvoicedAmount = ((BigDecimal) checkResult.get("invoicedTotal")).setScale(
							invoiceTypeDecimals, ROUNDING);
				} catch (GenericServiceException e) {
					Debug.logError(e, "Accounting trouble calling calculateInvoicedAdjustmentTotal service", module);
					return ServiceUtil.returnError(UtilProperties.getMessage(resource,
				...
				// If the absolute invoiced amount >= the abs of the adjustment
				// amount, the full amount has already been invoiced,
				// so skip this adjustment
				if (null == adj.get("amount")) { // JLR 17/4/7 : fix a bug
													// coming from POS in case
													// of use of a discount (on
													// item(s) or sale, sale
													// here) and a cash amount
													// higher than total (hence
													// issuing change)
					continue;
				}
				if (adjAlreadyInvoicedAmount.abs().compareTo(
			...
			// next do the shipping adjustments. Note that we do not want to add
			// these to the invoiceSubTotal or orderSubTotal for pro-rating tax
			// later, as that would cause
			// numerator/denominator problems when the shipping is not pro-rated
			// but rather charged all on the first invoice
			for (GenericValue adj : shipAdjustments.keySet()) {
				...
				if ("N".equalsIgnoreCase(prorateShipping)) {
					...
					// minus the total already invoiced for that adjustment,
					// since
			...
			List<GenericValue> orderPaymentPrefs = delegator.findList("OrderPaymentPreference",
					EntityCondition.makeCondition(paymentPrefConds, EntityOperator.AND), null, null, null, false);
			...
			// ProductStore.autoApproveInvoice setting, set status to
			// INVOICE_READY (unless it's a purchase invoice, which we set to
			// INVOICE_IN_PROCESS)
			...
			if (!"N".equals(autoApproveInvoice)) {
				...
				Map<String, Object> setInvoiceStatusResult = dispatcher.runSync("setInvoiceStatus", UtilMisc
						.<String, Object> toMap("invoiceId", invoiceId, "statusId", nextStatusId, "userLogin",
								userLogin));
	...
	public static Map<String, Object> createCommissionInvoices(DispatchContext dctx, Map<String, Object> context) {
		...
		for (String salesInvoiceId : salesInvoiceIds) {
			...
			try {
				...
				billFromVendorInvoiceRoles = EntityUtil.getFieldListFromEntityList(delegator.findList("InvoiceRole",
						EntityCondition.makeCondition(invoiceRoleConds, EntityOperator.AND), null, null, null, false),
						"partyId", true);
				...
				// commission invoices for all sales agent associated to sales
				// invoice.
				if (UtilValidate.isEmpty(salesRepPartyIds)) {
					salesRepPartyIds = EntityUtil.getFieldListFromEntityList(delegator.findList("InvoiceRole",
							EntityCondition.makeCondition(invoiceRoleConds, EntityOperator.AND), null, null, null,
							false), "partyId", true);
					if (UtilValidate.isEmpty(salesRepPartyIds)) {
						return ServiceUtil.returnError(UtilProperties.getMessage(resource,
					...
					List<String> salesInvoiceRolePartyIds = EntityUtil.getFieldListFromEntityList(delegator.findList(
							"InvoiceRole", EntityCondition.makeCondition(invoiceRoleConds, EntityOperator.AND), null,
							null, null, false), "partyId", true);
					if (UtilValidate.isNotEmpty(salesInvoiceRolePartyIds)) {
						salesRepPartyIds = UtilGenerics.checkList(CollectionUtils.intersection(salesRepPartyIds,
								salesInvoiceRolePartyIds));
					}
				}
				...
				invoiceItems = delegator.findList("InvoiceItem",
						EntityCondition.makeCondition("invoiceId", EntityOperator.EQUALS, salesInvoiceId), null, null,
						null, false);
			} catch (GenericEntityException e) {
				...
				if (UtilValidate.isNotEmpty(productId)) {
					...
					try {
						resultMap = dispatcher.runSync("getCommissionForProduct", UtilMisc.<String, Object> toMap(
								"productId", productId, "invoiceId", invoiceId, "invoiceItemSeqId", invoiceItemSeqId,
								"invoiceItemTypeId", invoiceItem.getString("invoiceItemTypeId"), "amount", amount,
					...
					// Note that getCommissionForProduct returns a List of Maps
					// with a lot values. See services.xml definition for
					// reference.
		...
		for (Map.Entry<String, List<Map<String, Object>>> commissionParty : commissionParties.entrySet()) {
			...
			// if there were days associated with the commission agreement, then
			// set a dueDate for the invoice.
			if (days != null) {
			...
			// create the bill-from (or pay-to) contact mech as the primary
			// PAYMENT_LOCATION of the party from the store
			List<EntityExpr> partyContactMechPurposeConds = UtilMisc.toList(EntityCondition.makeCondition("partyId",
					EntityOperator.EQUALS, partyIdBillTo), EntityCondition.makeCondition("contactMechPurposeTypeId",
			...
			try {
				partyContactMechPurposes = delegator.findList("PartyContactMechPurpose",
						EntityCondition.makeCondition(partyContactMechPurposeConds, EntityOperator.AND), null, null,
						null, false);
			} catch (GenericEntityException e) {
				return ServiceUtil.returnError(e.getMessage());
			}
			if (partyContactMechPurposes.size() > 0) {
				...
				GenericValue invoiceContactMech = delegator.makeValue("InvoiceContactMech", UtilMisc.toMap("invoiceId",
						invoiceId, "contactMechId", address.getString("contactMechId"), "contactMechPurposeTypeId",
						"BILLING_LOCATION"));
				toStore.add(invoiceContactMech);
			}
			partyContactMechPurposeConds = UtilMisc.toList(EntityCondition.makeCondition("partyId",
					EntityOperator.EQUALS, partyIdBillTo), EntityCondition.makeCondition("contactMechPurposeTypeId",
			...
			for (Map<String, Object> commissionMap : commList) {
				...
				try {
					resMap = dispatcher.runSync("createInvoiceItem", UtilMisc.toMap("invoiceId", invoiceId,
							"productId", commissionMap.get("productId"), "invoiceItemTypeId", "COMM_INV_ITEM",
							"quantity", quantity, "amount", elemAmount, "userLogin", userLogin));
					dispatcher.runSync("createInvoiceItemAssoc", UtilMisc.toMap("invoiceIdFrom", invoiceIdFrom,
							"invoiceItemSeqIdFrom", invoiceItemSeqIdFrom, "invoiceIdTo", invoiceId,
							"invoiceItemSeqIdTo", resMap.get("invoiceItemSeqId"), "invoiceItemAssocTypeId",
							"COMMISSION_INVOICE", "partyIdFrom", partyIdBillFrom, "partyIdTo", partyIdBillTo,
							"quantity", quantity, "amount", elemAmount, "userLogin", userLogin));
				} catch (GenericServiceException e) {
					return ServiceUtil.returnError(e.getMessage());
				}
				if (ServiceUtil.isError(resMap)) {
					return ServiceUtil.returnError(
							UtilProperties.getMessage(resource, "AccountingInvoiceCommissionErrorItem", locale), null,
							null, resMap);
				}
			}
			// store value objects
			try {
				...
				return ServiceUtil
						.returnError(UtilProperties.getMessage(resource,
								"AccountingInvoiceCommissionEntityDataProblem", UtilMisc.toMap("reason", e.toString()),
								locale));
			}
	...
	public static Map<String, Object> readyInvoices(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			for (String invoiceId : invoicesCreated) {
				...
				if (ServiceUtil.isError(setInvoiceStatusResult)) {
					return ServiceUtil.returnError(
							UtilProperties.getMessage(resource, "AccountingInvoiceCommissionError", locale), null,
							null, setInvoiceStatusResult);
				}
			}
		} catch (GenericServiceException e) {
			Debug.logError(e, "Entity/data problem creating commission invoice: " + e.toString(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource, "AccountingInvoiceCommissionError",
					UtilMisc.toMap("reason", e.toString()), locale));
		}
	...
	public static Map<String, Object> createInvoicesFromShipment(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			orderShipments = delegator
					.findByAnd("OrderShipment", UtilMisc.toMap("shipmentId", shipmentId), null, false);
		} catch (GenericEntityException e) {
			return ServiceUtil.returnError(e.getMessage());
		}
		...
		// Either no orderShipment exists, or there's a null invoicePerShipment
		// in the OrderHeader.
		...
		if ("Y".equals(invoicePerShipment)) {
			Map<String, Object> serviceContext = UtilMisc.toMap("shipmentIds", UtilMisc.toList(shipmentId),
					"eventDate", context.get("eventDate"), "userLogin", context.get("userLogin"));
			try {
	...
	public static Map<String, Object> setInvoicesToReadyFromShipment(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(resource,
					"AccountingTroubleGettingShipmentEntity", UtilMisc.toMap("shipmentId", shipmentId), locale));
		}
		...
		try {
			...
			itemIssuances = delegator.findList("ItemIssuance", EntityCondition.makeCondition("shipmentId", shipmentId),
					fieldsToSelect, UtilMisc.toList("orderId"), findOptions, false);
		} catch (GenericEntityException e) {
		...
		for (GenericValue itemIssuance : itemIssuances) {
			...
			try {
				...
				return ServiceUtil
						.returnError(UtilProperties.getMessage(resource, "AccountingProblemLookingUpOrderItemBilling",
		...
		for (GenericValue invoice : ordersWithInProcessInvoice.values()) {
			...
			try {
				setInvoiceStatusResult = dispatcher.runSync("setInvoiceStatus", UtilMisc.<String, Object> toMap(
						"invoiceId", invoiceId, "statusId", "INVOICE_READY", "userLogin", userLogin));
			} catch (GenericServiceException e) {
				Debug.logError(e, module);
				return ServiceUtil.returnError(e.getMessage());
			}
			if (ServiceUtil.isError(setInvoiceStatusResult)) {
				return ServiceUtil.returnError(
						UtilProperties.getMessage(resource, "AccountingErrorCreatingInvoiceFromOrder", locale), null,
						null, setInvoiceStatusResult);
			}
		}
		return ServiceUtil.returnSuccess();
	...
	public static Map<String, Object> createSalesInvoicesFromDropShipment(DispatchContext dctx,
		...
		Map<String, Object> serviceContext = UtilMisc.toMap("shipmentIds", UtilMisc.toList(shipmentId),
				"createSalesInvoicesForDropShipments", Boolean.TRUE, "userLogin", context.get("userLogin"));

		Map<String, Object> serviceResult;
		try {
			serviceResult = dispatcher.runSync("createInvoicesFromShipments", serviceContext);
		} catch (GenericServiceException e) {
			Debug.logError(e, "Trouble calling createInvoicesFromShipment service; invoice not created for shipment "
					+ shipmentId, module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource,
					"AccountingTroubleCallingCreateInvoicesFromShipmentService",
					UtilMisc.toMap("shipmentId", shipmentId), locale));
	...
	public static Map<String, Object> createInvoicesFromShipments(DispatchContext dctx,
		...
		// DEJ20060520: not used? planned to be used? List shipmentIdList = new
		// LinkedList();
		for (String tmpShipmentId : shipmentIds) {
			try {
				GenericValue shipment = delegator.findOne("Shipment", UtilMisc.toMap("shipmentId", tmpShipmentId),
						false);
				...
				if (purchaseShipmentFound && salesShipmentFound && dropShipmentFound) {
					return ServiceUtil.returnError(UtilProperties.getMessage(
							...
							UtilMisc.toMap("tmpShipmentId", tmpShipmentId, "shipmentTypeId",
									shipment.getString("shipmentTypeId")), locale));
				}
			} catch (GenericEntityException e) {
				Debug.logError(e, "Trouble getting Shipment entity for shipment " + tmpShipmentId, module);
				return ServiceUtil.returnError(UtilProperties.getMessage(resource,
						"AccountingTroubleGettingShipmentEntity", UtilMisc.toMap("tmpShipmentId", tmpShipmentId),
						locale));
			}
		}
		...
		// get the items of the shipment. They can come from ItemIssuance if the
		// shipment were from a sales order, ShipmentReceipt
		// if it were a purchase order or from the order items of the (possibly
		// linked) orders if the shipment is a drop shipment
		List<GenericValue> items = null;
		List<GenericValue> orderItemAssocs = null;
		try {
			if (purchaseShipmentFound) {
				items = delegator.findList("ShipmentReceipt", shipmentIdsCond, null, UtilMisc.toList("shipmentId"),
						null, false);
				// filter out items which have been received but are not
				// actually owned by an internal organization, so they should
				// not be on a purchase invoice
				...
				while (itemsIter.hasNext()) {
					...
					GenericValue ownerPartyRole = delegator.findOne("PartyRole", UtilMisc.toMap("partyId",
							inventoryItem.getString("ownerPartyId"), "roleTypeId", "INTERNAL_ORGANIZATIO"), true);
				...
				List<String> purchaseOrderIds = EntityUtil
						.getFieldListFromEntityList(shipments, "primaryOrderId", true);

				if (createSalesInvoicesForDropShipments) {

					// If a sales invoice is being created for a drop shipment,
					// we have to reference the original sales order items
					// Get the list of the linked orderIds (original sales
					// orders)
					orderItemAssocs = delegator.findList("OrderItemAssoc",
							EntityCondition.makeCondition("toOrderId", EntityOperator.IN, purchaseOrderIds), null,
							null, null, false);

					// Get only the order items which are indirectly related to
					// the purchase order - this limits the list to the drop
					// ship group(s)
					items = EntityUtil.getRelated("FromOrderItem", null, orderItemAssocs, false);
				} else {

					// If it's a purchase invoice being created, the order items
					// for that purchase orders can be used directly
					items = delegator.findList("OrderItem",
							EntityCondition.makeCondition("orderId", EntityOperator.IN, purchaseOrderIds), null, null,
							null, false);
				}
			} else {
				items = delegator.findList("ItemIssuance", shipmentIdsCond, null, UtilMisc.toList("shipmentId"), null,
						false);
			}
		} catch (GenericEntityException e) {
			Debug.logError(e, "Problem getting issued items from shipments", module);
		...
		for (GenericValue item : items) {
			...
			try {
				itemBillings = delegator.findList("OrderItemBillingAndInvoiceAndItem",
						EntityCondition.makeCondition(billFields, EntityOperator.AND), null, null, null, false);
			} catch (GenericEntityException e) {
				Debug.logError(e, "Problem looking up OrderItemBilling records for " + billFields, module);
				return ServiceUtil
						.returnError(UtilProperties.getMessage(resource, "AccountingProblemLookingUpOrderItemBilling",
		...
		// make sure we aren't billing items already invoiced i.e. items billed
		// as digital (FINDIG)
		...
		for (String orderId : orders) {
			...
			// now we will check each issuance and make sure it hasn't already
			// been billed
			for (GenericValue issue : billItems) {
				...
				if (billAvail == null) {
					...
					try {
						orderItem = issue.getEntityName().equals("OrderItem") ? issue : issue.getRelatedOne(
								"OrderItem", false);
						...
						if (dropShipmentFound && createSalesInvoicesForDropShipments.booleanValue()) {
							...
							// Override the issueQty with the quantity from the
							// purchase order item
						...
						billed = delegator.findList("OrderItemBillingAndInvoiceAndItem",
								EntityCondition.makeCondition(lookup, EntityOperator.AND), null, null, null, false);
					} catch (GenericEntityException e) {
				...
				if (billAvail != null && billAvail.signum() == 1) { // this
																	// checks if
																	// billAvail
																	// is a
																	// positive
					...
					if (issueQty != null && issueQty.compareTo(billAvail) > 0) {
						// can only bill some of the issuance; others have been
						// billed already
			...
			// If shipping charges are not prorated, the shipments need to be
			// examined for additional shipping charges
			if ("N".equalsIgnoreCase(prorateShipping)) {
				...
				try {
					if (dropShipmentFound) {
						...
						if (createSalesInvoicesForDropShipments) {
							...
							// If a sales invoice is being created for the drop
							// shipment, we need to reference back to the
							// original purchase order IDs

							// Get the IDs for orders which have billable items
							List<String> invoiceableLinkedOrderIds = EntityUtil.getFieldListFromEntityList(toBillItems,
									"orderId", true);

							// Get back the IDs of the purchase orders - this
							// will be a list of the purchase order items which
							// are billable by virtue of not having been
							...
							List<GenericValue> reverseOrderItemAssocs = EntityUtil.filterByCondition(orderItemAssocs,
									EntityCondition.makeCondition("orderId", EntityOperator.IN,
											invoiceableLinkedOrderIds));
							invoiceablePrimaryOrderIds = EntityUtil.getFieldListFromEntityList(reverseOrderItemAssocs,
									"toOrderId", true);

						} else {

							// If a purchase order is being created for a drop
							// shipment, the purchase order IDs can be used
							// directly
							invoiceablePrimaryOrderIds = EntityUtil.getFieldListFromEntityList(toBillItems, "orderId",
									true);
						...
						if (!UtilValidate.isEmpty(invoiceablePrimaryOrderIds)) {
							List<EntityExpr> invoiceableShipmentConds = UtilMisc.toList(EntityCondition.makeCondition(
									"primaryOrderId", EntityOperator.IN, invoiceablePrimaryOrderIds), EntityCondition
									.makeCondition("shipmentId", EntityOperator.IN, shipmentIds));
							invoiceableShipments = delegator.findList("Shipment",
									EntityCondition.makeCondition(invoiceableShipmentConds, EntityOperator.AND), null,
									null, null, false);
						}
					} else {
						...
						if (UtilValidate.isNotEmpty(invoiceableShipmentIds)) {
							invoiceableShipments = delegator.findList("Shipment", EntityCondition.makeCondition(
									"shipmentId", EntityOperator.IN, invoiceableShipmentIds), null, null, null, false);
						}
					}
				} catch (GenericEntityException e) {
				...
				if (UtilValidate.isNotEmpty(invoiceableShipments)) {
					for (GenericValue shipment : invoiceableShipments) {
						if (shipment.get("additionalShippingCharge") == null)
							continue;
						BigDecimal shipmentAdditionalShippingCharges = shipment.getBigDecimal(
								"additionalShippingCharge").setScale(DECIMALS, ROUNDING);
				...
				if (totalAdditionalShippingCharges.signum() == 1) {
					...
					for (Map.Entry<GenericValue, BigDecimal> entry : additionalShippingCharges.entrySet()) {
						...
						try {
							Map<String, Object> createOrderAdjustmentResult = dispatcher.runSync(
									"createOrderAdjustment", createOrderAdjustmentContext);
						...
						// These parameters don't matter if we're only worried
						// about adjustments on the shipping charges
						...
						List<GenericValue> orderAdjustments = UtilGenerics.checkList(calcTaxResult
						...
						if (orderAdjustments != null) {
							for (GenericValue orderAdjustment : orderAdjustments) {
								totalAdditionalShippingCharges = totalAdditionalShippingCharges.add(orderAdjustment
						...
						// If part of the order was paid via credit card, try to
						// charge it for the additional shipping
						List<GenericValue> orderPaymentPreferences = null;
						try {
							orderPaymentPreferences = delegator.findByAnd("OrderPaymentPreference",
									UtilMisc.toMap("orderId", orderId, "paymentMethodTypeId", "CREDIT_CARD"), null,
									false);
						} catch (GenericEntityException e) {
						...
						// Use the first credit card we find, for the sake of
						// simplicity
						...
						if (paymentMethodId != null) {
							...
							// Release all outstanding (not settled or
							// cancelled) authorizations, while keeping a
							// running
							// total of their amounts so that the total plus the
							// additional shipping charges can be authorized
							// again
							// all at once.
							...
							for (GenericValue orderPaymentPreference : orderPaymentPreferences) {
								if (!(orderPaymentPreference.getString("statusId").equals("PAYMENT_SETTLED") || orderPaymentPreference
										.getString("statusId").equals("PAYMENT_CANCELLED"))) {
									...
									if (authTransaction != null && authTransaction.get("amount") != null) {
										...
										totalNewAuthAmount = totalNewAuthAmount.add(authTransaction.getBigDecimal(
												"amount").setScale(DECIMALS, ROUNDING));

										// Release the authorization for the
										// OrderPaymentPreference
										Map<String, Object> prefReleaseResult = null;
										try {
											prefReleaseResult = dispatcher.runSync("releaseOrderPaymentPreference",
													UtilMisc.toMap("orderPaymentPreferenceId", orderPaymentPreference
															.getString("orderPaymentPreferenceId"), "userLogin",
															context.get("userLogin")));
										} catch (GenericServiceException e) {
											Debug.logError(e, "Trouble calling releaseOrderPaymentPreference service",
													module);
											return ServiceUtil.returnError(UtilProperties.getMessage(resource,
							...
							// Create a new OrderPaymentPreference for the order
							// to handle the new (totalled) charge. Don't
							// set the maxAmount so that it doesn't interfere
							// with other authorizations
							Map<String, Object> serviceContext = UtilMisc.toMap("orderId", orderId, "paymentMethodId",
									paymentMethodId, "paymentMethodTypeId", "CREDIT_CARD", "userLogin",
									context.get("userLogin"));
							...
							// Attempt to authorize the new
							// orderPaymentPreference
							Map<String, Object> authResult = null;
							try {
								// Use an overrideAmount because the maxAmount
								// wasn't set on the OrderPaymentPreference
								authResult = dispatcher.runSync("authOrderPaymentPreference", UtilMisc.toMap(
										"orderPaymentPreferenceId", orderPaymentPreferenceId, "overrideAmount",
										totalNewAuthAmount, "userLogin", context.get("userLogin")));
							} catch (GenericServiceException e) {
							...
							if (authErrors || !authFinished) {
								String errMsg = UtilProperties.getMessage(resource,
										"AccountingUnableToAuthAdditionalShipCharges", UtilMisc.toMap("shipmentId",
												shipmentId, "paymentMethodId", paymentMethodId,
												"orderPaymentPreferenceId", orderPaymentPreferenceId), locale);
								Debug.logError(errMsg, module);
							}
				...
				Debug.logInfo(
						UtilProperties.getMessage(resource, "AccountingIgnoringAdditionalShipCharges",
		...
		try {
			...
			if (shipment == null) {
				return ServiceUtil.returnError(errorMsg
						+ UtilProperties.getMessage(resource, "AccountingShipmentNotFound", locale));
			}
			...
			if (!(salesReturnFound || purchaseReturnFound)) {
				return ServiceUtil.returnError(errorMsg
						+ UtilProperties.getMessage(resource, "AccountingShipmentNotSalesReturnAndPurchaseReturn",
								locale));
			}
			// get the items of the shipment. They can come from ItemIssuance if
			// the shipment were from a purchase return, ShipmentReceipt if it
			...
			for (GenericValue item : shippedItems) {
				...
				if (item.getEntityName().equals("ShipmentReceipt")) {
					...
					GenericValue returnItemShipment = EntityUtil.getFirst(delegator.findByAnd(
							"ReturnItemShipment",
							UtilMisc.toMap("shipmentId", item.getString("shipmentId"), "shipmentItemSeqId",
									item.getString("shipmentItemSeqId")), null, false));
				...
				if (item.getEntityName().equals("ShipmentReceipt")) {
					billings = delegator.findByAnd("ReturnItemBilling", UtilMisc.toMap("shipmentReceiptId",
							item.getString("receiptId"), "returnId", returnId, "returnItemSeqId",
							item.get("returnItemSeqId")), null, false);
				} else if (item.getEntityName().equals("ItemIssuance")) {
					billings = delegator.findByAnd("ReturnItemBilling",
							UtilMisc.toMap("returnId", returnId, "returnItemSeqId", returnItemSeqId), null, false);
				}
			...
			// loop through the returnId keys in the map and invoke the
			// createInvoiceFromReturn service for each
			for (Map.Entry<String, List<GenericValue>> entry : itemsShippedGroupedByReturn.entrySet()) {
				...
				if (Debug.verboseOn()) {
					Debug.logVerbose(
							"Creating invoice for return [" + returnId + "] with items: " + billItems.toString(),
							module);
				}
				Map<String, Object> input = UtilMisc.toMap("returnId", returnId, "billItems", billItems, "userLogin",
						context.get("userLogin"));
	...
	public static Map<String, Object> createInvoiceFromReturn(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			// keep track of the invoice total vs the promised return total (how
			// much the customer promised to return)
			BigDecimal invoiceTotal = ZERO;
			BigDecimal promisedTotal = ZERO;

			// loop through shipment receipts to create invoice items and return
			// item billings for each item and adjustment
			int invoiceItemSeqNum = 1;
			String invoiceItemSeqId = UtilFormatOut.formatPaddedNumber(invoiceItemSeqNum,
					INVOICE_ITEM_SEQUENCE_ID_DIGITS);
			...
			for (GenericValue item : billItems) {
				...
				if (invoiceItemTypeId == null) {
					return ServiceUtil.returnError(errorMsg
							+ UtilProperties.getMessage(resource, "AccountingNoKnownInvoiceItemTypeReturnItemType",
				...
				input = UtilMisc.toMap("invoiceId", invoiceId, "invoiceItemTypeId", invoiceItemTypeId, "quantity",
						quantity);
				input.put("invoiceItemSeqId", "" + invoiceItemSeqId); // turn
																		// the
																		// int
																		// into
																		// a
				...
				if (Debug.verboseOn()) {
					Debug.logVerbose(
							"Creating Invoice Item with amount " + returnPrice + " and quantity " + quantity
									+ " for shipment [" + item.getString("shipmentId") + ":"
									+ item.getString("shipmentItemSeqId") + "]", module);
				}

				String parentInvoiceItemSeqId = invoiceItemSeqId;
				// increment the seqId counter after creating the invoice item
				// and return item billing
				invoiceItemSeqNum += 1;
				invoiceItemSeqId = UtilFormatOut.formatPaddedNumber(invoiceItemSeqNum, INVOICE_ITEM_SEQUENCE_ID_DIGITS);

				// keep a running total (note: a returnItem may have many
				// receipts. hence, the promised total quantity is the receipt
				...
				// for each adjustment related to this ReturnItem, create a
				...
				for (GenericValue adjustment : adjustments) {
					...
					// determine invoice item type from the return item type
					invoiceItemTypeId = getInvoiceItemType(delegator, adjustment.getString("returnAdjustmentTypeId"),
							null, invoiceTypeId, null);
					if (invoiceItemTypeId == null) {
						return ServiceUtil.returnError(errorMsg
								+ UtilProperties.getMessage(
										...
										UtilMisc.toMap("returnAdjustmentTypeId",
												adjustment.getString("returnAdjustmentTypeId")), locale));
					}

					// prorate the adjustment amount by the returned amount; do
					// not round ratio
					...
					if (Debug.verboseOn()) {
						Debug.logVerbose(
								"Creating Invoice Item with amount " + adjustment.getBigDecimal("amount")
					...
					// TODO: Note that we use the value of Product.taxable here.
					// This is not an ideal solution. Instead, use
					// returnAdjustment.includeInTax
					...
					// keep a running total (promised adjustment in this case is
					// the same as the invoice adjustment)
			...
			// ratio of the invoice total to the promised total so far or zero
			// if the amounts were zero
			BigDecimal actualToPromisedRatio = ZERO;
			if (invoiceTotal.signum() != 0) {
				actualToPromisedRatio = invoiceTotal.divide(promisedTotal, 100, ROUNDING); // do
																							// not
																							// round
																							// ratio
			}

			// loop through return-wide adjustments and create invoice items for
			// each
			...
			for (GenericValue adjustment : adjustments) {
				...
				if (Debug.verboseOn()) {
					Debug.logVerbose(
							"Creating Invoice Item with amount " + adjustment.getBigDecimal("amount") + " prorated to "
									+ amount + " for return adjustment [" + adjustment.getString("returnAdjustmentId")
									+ "]", module);
				}
				...
				// XXX TODO Note: we need to implement
				// ReturnAdjustment.includeInTax for this to work properly
	...
	public static Map<String, Object> checkInvoicePaymentApplications(DispatchContext ctx, Map<String, Object> context) {
		...
		try {
			paymentAppl = delegator.findByAnd("PaymentAndApplication", UtilMisc.toMap("invoiceId", invoiceId), null,
					false);
			// For each payment application, select only those that are RECEIVED
			// or SENT based on whether the payment is a RECEIPT or DISBURSEMENT
			// respectively
		...
		if (totalPayments.signum() == 1) {
			...
			if (totalPayments.compareTo(invoiceTotal) >= 0) { // this checks
																// that
																// totalPayments
																// is greater
																// than or equal
				...
				try {
					...
					return ServiceUtil.returnError(UtilProperties.getMessage(resource,
							"AccountingProblemChangingInvoiceStatusTo", UtilMisc.toMap("newStatus", "INVOICE_PAID"),
							locale));
				}
			}
		} else {
	...
	private static BigDecimal calcHeaderAdj(Delegator delegator, GenericValue adj, String invoiceTypeId,
		...
		if (adj.get("amount") != null) {
			...
			// pro-rate the amount
			BigDecimal amount = ZERO;
			// make sure the divisor is not 0 to avoid NaN problems; just leave
			// the amount as 0 and skip it in essense
			if (divisor.signum() != 0) {
			...
			if (amount.signum() != 0) {
				...
				createInvoiceItemContext.put(
						"invoiceItemTypeId",
						getInvoiceItemType(delegator, adj.getString("orderAdjustmentTypeId"), null, invoiceTypeId,
								"INVOICE_ADJ"));
		...
		Debug.logInfo("adjAmount: " + adjAmount + ", divisor: " + divisor + ", multiplier: " + multiplier
				...
				+ invoiceItemSeqId + ", decimals: " + decimals + ", rounding: " + rounding + ", adj: " + adj, module);
		return adjAmount;
	}

	/*
	 * Creates InvoiceTerm entries for a list of terms, which can be
	 * BillingAccountTerms, OrderTerms, etc.
	 */
	private static void createInvoiceTerms(Delegator delegator, LocalDispatcher dispatcher, String invoiceId,
			List<GenericValue> terms, GenericValue userLogin, Locale locale) {
		if (terms != null) {
	 ...
	 * it works on the invoice item level.
	 * 
	 * This version will apply as much as possible when no amountApplied is
	 * provided.
	 */
	...
	public static Map<String, Object> updatePaymentApplicationDefBd(DispatchContext dctx, Map<String, Object> context) {
		...
		// id is provided not 2,3... BUT a combination of billingAccountId and
		// invoiceId is permitted - that's how you use a
		...
		// retrieve all information and perform checking on the retrieved
		// info.....
		...
		// Payment this should be checked after the invoice checking because it
		// is possible the currency is changed
		...
		if (paymentId == null || paymentId.equals("")) {
			...
			paymentApplyAvailable = payment.getBigDecimal("amount").subtract(PaymentWorker.getPaymentApplied(payment))
					.setScale(DECIMALS, ROUNDING);
		...
		if (toPaymentId != null && !toPaymentId.equals("")) {
			...
			// check if at least one send is the same as one receiver on the
			// other payment
		...
		// assign payment to billing account if the invoice is assigned to this
		// billing account
		if (invoiceId != null) {
			GenericValue invoice = null;
			try {
		...
		// amount available on the invoice reduced by the already applied
		// amounts
		BigDecimal invoiceItemApplyAvailable = ZERO;
		// amount available on the invoiceItem reduced by the already applied
		// amounts
		GenericValue invoice = null;
		...
		if (invoiceId != null) {
			...
			if (invoice == null) {
				...
				if (invoice.getString("statusId").equals("INVOICE_CANCELLED")) {
					errorMessageList.add(UtilProperties.getMessage(resource, "AccountingInvoiceCancelledCannotApplyTo",
							UtilMisc.toMap("invoiceId", invoiceId), locale));
				}
				...
				if (currencyUomId != null && invoice.get("currencyUomId") != null
					...
					Debug.logInfo(UtilProperties.getMessage(resource, "AccountingInvoicePaymentCurrencyProblem",
							UtilMisc.toMap("invoiceCurrency", invoice.getString("currencyUomId"), "paymentCurrency",
									payment.getString("currencyUomId")), locale), module);
					...
					if (payment.get("actualCurrencyAmount") == null || payment.get("actualCurrencyUomId") == null) {
						errorMessageList
								.add("Actual amounts are required in the currency of the invoice to make this work....");
					} else {
				...
				if (invoiceTotal.signum() == 0) {
					...
					// only check for new records here...updates are checked in
					// the paymentApplication section
					if (invoiceApplyAvailable.signum() == 0) {
						errorMessageList.add(UtilProperties.getMessage(resource, "AccountingInvoiceCompletelyApplied",
								UtilMisc.toMap("invoiceId", invoiceId), locale));
					}
					// check here for too much application if a new record(s)
					// are
					// added (paymentApplicationId == null)
						...
						errorMessageList.add(UtilProperties.getMessage(resource, "AccountingInvoiceLessRequested",
								UtilMisc.<String, Object> toMap("invoiceId", invoiceId, "invoiceApplyAvailable",
										...
										invoice.getString("currencyUomId")), locale));
					}
				}

				// check if at least one sender is the same as one receiver on
				// the invoice
			...
			if (invoiceItemSeqId != null) {
				// when itemSeqNr not provided delay checking on
				// invoiceItemSeqId
				try {
				...
				if (invoiceItem == null) {
					...
					if (invoice.get("currencyUomId") != null && currencyUomId != null
						...
						errorMessageList.add(UtilProperties.getMessage(
								resource,
								"AccountingInvoicePaymentCurrencyProblem",
								UtilMisc.toMap("paymentCurrencyId", currencyUomId, "itemCurrency",
										invoice.getString("currencyUomId")), locale));
					}
					...
					invoiceItemApplyAvailable = invoiceItem.getBigDecimal("amount").multiply(quantity)
							.setScale(DECIMALS, ROUNDING).subtract(InvoiceWorker.getInvoiceItemApplied(invoiceItem));
					// check here for too much application if a new record is
					// added
		...
		// check this at the end because the invoice can change the
		// currency.......
		if (paymentApplicationId == null) {
			// only check for new application records, update on existing
			// records is checked in the paymentApplication section
			if (paymentApplyAvailable.signum() == 0) {
				...
				if (amountApplied.compareTo(paymentApplyAvailable) > 0) {
					errorMessageList.add(UtilProperties.getMessage(resource, "AccountingPaymentLessRequested", UtilMisc
							.<String, Object> toMap("paymentId", paymentId, "paymentApplyAvailable",
									paymentApplyAvailable, "amountApplied", amountApplied, "isoCode", currencyUomId),
							locale));
				}
			}
		}
		...
		// amount available on the invoice taking into account if the
		// invoiceItemnumber has changed
		BigDecimal newInvoiceItemApplyAvailable = invoiceItemApplyAvailable;
		// amount available on the invoiceItem taking into account if the
		// itemnumber has changed
		...
		if (paymentApplicationId == null) {
			...
			if (paymentApplication == null) {
				...
				// obviously a change
				// only take the newly entered item, same for tax authority and
				// toPayment
				...
				if (paymentApplyAvailable.compareTo(ZERO) == 0) {
					...
					newPaymentApplyAvailable = paymentApplyAvailable.add(paymentApplyAvailable).subtract(amountApplied)
							.setScale(DECIMALS, ROUNDING);
				}
				if (newPaymentApplyAvailable.compareTo(ZERO) < 0) {
					errorMessageList.add(UtilProperties.getMessage(resource, "AccountingPaymentNotEnough", UtilMisc
							.<String, Object> toMap("paymentId", paymentId, "paymentApplyAvailable",
				...
				if (invoiceId != null) {
					// only when we are processing an invoice on existing
					// paymentApplication check invoice item for to much
					// application if the invoice
					...
					if (invoiceId.equals(paymentApplication.getString("invoiceId"))) {
						...
						if (invoiceItemSeqId == null && paymentApplication.get("invoiceItemSeqId") == null) {
							...
							// check if the item number changed from a real Item
							// number to a null value
							newInvoiceApplyAvailable = invoiceApplyAvailable
									.add(paymentApplication.getBigDecimal("amountApplied")).subtract(amountApplied)
									.setScale(DECIMALS, ROUNDING);
							if (invoiceApplyAvailable.compareTo(ZERO) < 0) {
							...
							// value to
							// a real Item number
							newInvoiceItemApplyAvailable = invoiceItemApplyAvailable.subtract(amountApplied).setScale(
									DECIMALS, ROUNDING);
							if (newInvoiceItemApplyAvailable.compareTo(ZERO) < 0) {
								errorMessageList.add(UtilProperties.getMessage(resource,
										"AccountingItemInvoiceNotEnough", UtilMisc.<String, Object> toMap("tooMuch",
												newInvoiceItemApplyAvailable.negate(), "invoiceId", invoiceId,
							...
							newInvoiceItemApplyAvailable = invoiceItemApplyAvailable.add(amountApplied).setScale(
									DECIMALS, ROUNDING);
							if (newInvoiceItemApplyAvailable.compareTo(ZERO) < 0) {
								errorMessageList.add(UtilProperties.getMessage(resource,
										"AccountingItemInvoiceNotEnough", UtilMisc.<String, Object> toMap("tooMuch",
						...
						// check the invoice
						newInvoiceApplyAvailable = invoiceApplyAvailable.add(
								paymentApplication.getBigDecimal("amountApplied").subtract(amountApplied)).setScale(
								DECIMALS, ROUNDING);
							...
							errorMessageList.add(UtilProperties.getMessage(resource, "AccountingInvoiceNotEnough",
									UtilMisc.<String, Object> toMap(
											...
											invoiceApplyAvailable
													.add(paymentApplication.getBigDecimal("amountApplied")).subtract(
															amountApplied), "invoiceId", invoiceId), locale));
						}
					}
				}
				...
				if (toPaymentId != null && toPaymentId.equals(paymentApplication.getString("toPaymentId"))) {
					...
					if (newToPaymentApplyAvailable.compareTo(ZERO) < 0) {
						errorMessageList.add(UtilProperties.getMessage(resource, "AccountingPaymentNotEnough", UtilMisc
								.<String, Object> toMap("paymentId", toPaymentId, "paymentApplyAvailable",
										newToPaymentApplyAvailable, "amountApplied", amountApplied), locale));
					}
		...
		if (debug) {
			...
			if (invoiceId != null) {
				Debug.logInfo(" Invoice(" + invoiceId + ") amount not yet applied: " + newInvoiceApplyAvailable + extra
						+ " Payment(" + paymentId + ") amount not yet applied: " + newPaymentApplyAvailable
			...
			if (toPaymentId != null) {
				Debug.logInfo(" toPayment(" + toPaymentId + ") amount not yet applied: " + newToPaymentApplyAvailable
						+ " Payment(" + paymentId + ") amount not yet applied: " + newPaymentApplyAvailable
						+ " Requested amount to apply:" + amountApplied, module);
			...
			if (taxAuthGeoId != null) {
				Debug.logInfo(" taxAuthGeoId(" + taxAuthGeoId + ")  Payment(" + paymentId
						+ ") amount not yet applied: " + newPaymentApplyAvailable + " Requested amount to apply:"
						+ amountApplied, module);
				toMessage = UtilProperties.getMessage(resource, "AccountingApplicationToTax",
						UtilMisc.toMap("taxAuthGeoId", taxAuthGeoId), locale);
			}
		}
		// if the amount to apply was not provided or was zero fill it with the
		// maximum possible and provide information to the user
		...
		if (amountApplied.signum() == 0) {
			...
			successMessage = UtilProperties.getMessage(resource, "AccountingApplicationSuccess", UtilMisc
					.<String, Object> toMap("amountApplied", amountApplied, "paymentId", paymentId, "isoCode",
							currencyUomId, "toMessage", toMessage), locale);
		}
		...
		// ============ start processing ======================
		// if the application is specified it is easy, update the existing
		// record only
		if (paymentApplicationId != null) {
		...
		// if no invoice sequence number is provided it assumed the requested
		// paymentAmount will be
		...
		if (invoiceId != null && paymentId != null && (invoiceItemSeqId == null)) {
			if (invoiceProcessing) {
				...
				try {
					invoiceItems = delegator.findByAnd("InvoiceItem", UtilMisc.toMap("invoiceId", invoiceId), null,
							false);
				} catch (GenericEntityException e) {
					return ServiceUtil.returnError(e.getMessage());
				}
				if (invoiceItems.size() == 0) {
					errorMessageList.add(UtilProperties.getMessage(resource, "AccountingNoInvoiceItemsFoundForInvoice",
							UtilMisc.toMap("invoiceId", invoiceId), locale));
					return ServiceUtil.returnError(errorMessageList);
				} else { // we found some invoice items, start processing....
					// check if the user want to apply a smaller amount than the
					// maximum possible on the payment
					...
					for (GenericValue currentInvoiceItem : invoiceItems) {
						...
						if (debug)
							Debug.logInfo("Start processing item: " + currentInvoiceItem.getString("invoiceItemSeqId"),
									module);
						...
						if (currentInvoiceItem.get("quantity") != null
							...
							itemQuantity = new BigDecimal(currentInvoiceItem.getString("quantity")).setScale(DECIMALS,
									ROUNDING);
						}
						...
						try {
							paymentApplications = currentInvoiceItem
									.getRelated("PaymentApplication", null, null, false);
						} catch (GenericEntityException e) {
							return ServiceUtil.returnError(e.getMessage());
						}
						...
						if (UtilValidate.isNotEmpty(paymentApplications)) {
							...
							while (p.hasNext()) {
								...
								alreadyApplied = alreadyApplied.add(paymentApplication.getBigDecimal("amountApplied")
										.setScale(DECIMALS, ROUNDING));
							}
						...
						if (tobeApplied.signum() == 0) {
							// invoiceItem already fully applied so look at the
							// next one....
							continue;
						}
						...
						if (invoice.get("currencyUomId") != null && currencyUomId != null
							...
							paymentApplication.set("invoiceId", invoiceId);
							paymentApplication
									.set("invoiceItemSeqId", currentInvoiceItem.getString("invoiceItemSeqId"));
							paymentApplication.set("paymentId", paymentId);
							paymentApplication.set("toPaymentId", toPaymentId);
						...
						/*
						 * if (InvoiceWorker.getInvoiceTotal(invoice).equals(
						 * InvoiceWorker.getInvoiceApplied(invoice))) { try {
						 * dispatcher.runSync("setInvoiceStatus",
						 * UtilMisc.toMap(
						 * "invoiceId",invoiceId,"statusId","INVOICE_PAID")); }
						 * catch (GenericServiceException e1) {
						 * Debug.logError(e1, "Error updating invoice status",
						 * module); } }
						 * 
						 * if
						 * (payment.getBigDecimal("amount").equals(PaymentWorker
						 * .getPaymentApplied(payment))) { GenericValue
						 * appliedPayment = (GenericValue)
						 * delegator.makeValue("Payment",
						 * UtilMisc.toMap("paymentId"
						 * ,paymentId,"statusId","INVOICE_PAID")); try {
						 * appliedPayment.store(); } catch
		...
		errorMessageList.add(UtilProperties.getMessage(resource, "AccountingPaymentApplicationParameterUnsuitable",
				locale));
		errorMessageList.add(UtilProperties.getMessage(resource, "AccountingPaymentApplicationParameterListUnsuitable",
				UtilMisc.toMap("invoiceId", invoiceId, "invoiceItemSeqId", invoiceItemSeqId, "paymentId", paymentId,
						"toPaymentId", toPaymentId, "paymentApplicationId", paymentApplicationId, "amountApplied",
						amountApplied), locale));
		return ServiceUtil.returnError(errorMessageList);
	}
	...
	public static Map<String, Object> calculateInvoicedAdjustmentTotal(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			invoicedAdjustments = delegator.findByAnd("OrderAdjustmentBilling",
					UtilMisc.toMap("orderAdjustmentId", orderAdjustment.getString("orderAdjustmentId")), null, false);
		} catch (GenericEntityException e) {
			Debug.logError(e, "Accounting trouble calling calculateInvoicedAdjustmentTotal service", module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource,
	 ...
	 * @return map results
	 */
	private static Map<String, Object> storePaymentApplication(Delegator delegator, GenericValue paymentApplication,
			Locale locale) {
		...
		try {
			checkAppls = delegator.findByAnd("PaymentApplication", UtilMisc.toMap("invoiceId",
					paymentApplication.get("invoiceId"), "invoiceItemSeqId",
					paymentApplication.get("invoiceItemSeqId"), "billingAccountId",
					paymentApplication.get("billingAccountId"), "paymentId", paymentApplication.get("paymentId"),
					"toPaymentId", paymentApplication.get("toPaymentId"), "taxAuthGeoId",
					paymentApplication.get("taxAuthGeoId")), null, false);
		} catch (GenericEntityException e) {
			return ServiceUtil.returnError(e.getMessage());
		}
		if (checkAppls.size() > 0) {
			...
			if (paymentApplication.get("paymentApplicationId") == null) {
				...
				checkAppl.set("amountApplied",
						paymentApplication.getBigDecimal("amountApplied").add(checkAppl.getBigDecimal("amountApplied"))
								.setScale(DECIMALS, ROUNDING));
				if (debug)
					Debug.logInfo("Update paymentApplication record: " + checkAppl.getString("paymentApplicationId")
							+ " with appliedAmount:" + checkAppl.getBigDecimal("amountApplied"), module);
				try {
					checkAppl.store();
				} catch (GenericEntityException e) {
					return ServiceUtil.returnError(e.getMessage());
				}
			} else if (paymentApplication.getString("paymentApplicationId").equals(
					checkAppl.getString("paymentApplicationId"))) {
				...
				checkAppl.set("amountApplied",
						paymentApplication.getBigDecimal("amountApplied").add(checkAppl.getBigDecimal("amountApplied"))
								.setScale(DECIMALS, ROUNDING));
				// delete paymentApplication record and update the checkAppls
				// one.
				if (debug)
					Debug.logInfo(
							"Delete paymentApplication record: " + paymentApplication.getString("paymentApplicationId")
									+ " with appliedAmount:" + paymentApplication.getBigDecimal("amountApplied"),
							module);
				try {
			...
			if (paymentApplication.get("paymentApplicationId") == null) {
				...
				if (debug)
					Debug.logInfo(
							"Create new paymentAppication record: "
									+ paymentApplication.getString("paymentApplicationId") + " with appliedAmount:"
									+ paymentApplication.getBigDecimal("amountApplied"), module);
				try {
				...
				// existing combination of paymentId/invoiceId/invoiceSeqId/
				// etc... was provided
				if (debug)
					Debug.logInfo(
							"Update existing paymentApplication record: "
									+ paymentApplication.getString("paymentApplicationId") + " with appliedAmount:"
									+ paymentApplication.getBigDecimal("amountApplied"), module);
				try {
	...
	public static Map<String, Object> checkPaymentInvoices(DispatchContext dctx, Map<String, Object> context) {
		...
		try {
			...
			// TODO: this is inefficient -- instead use HashSet to construct a
			// distinct Set of invoiceIds, then iterate over it and call
			// checkInvoicePaymentAppls
			for (GenericValue paymentApplication : paymentApplications) {
				String invoiceId = paymentApplication.getString("invoiceId");
				if (invoiceId != null) {
					Map<String, Object> serviceResult = dispatcher.runSync("checkInvoicePaymentApplications",
							UtilMisc.<String, Object> toMap("invoiceId", invoiceId, "userLogin", userLogin));
					...
					try {
						...
						if (UtilValidate.isEmpty(invoice.get("partyIdFrom"))) {
							newErrMsgs.add("Line number " + rec.getRecordNumber()
								...
								UtilMisc.<String, Object> toMap("partyId", invoice.get("partyIdFrom")), false) == null) {
						...
						if (UtilValidate.isEmpty(invoice.get("partyId"))) {
							newErrMsgs.add("Line number " + rec.getRecordNumber()
								...
								UtilMisc.<String, Object> toMap("partyId", invoice.get("partyId")), false) == null) {
						...
						if (UtilValidate.isEmpty(invoice.get("invoiceTypeId"))) {
							newErrMsgs.add("Line number " + rec.getRecordNumber()
								...
								UtilMisc.<String, Object> toMap("invoiceTypeId", invoice.get("invoiceTypeId")), false) == null) {
						...
						GenericValue invoiceType = delegator.findOne("InvoiceType",
								UtilMisc.<String, Object> toMap("invoiceTypeId", invoice.get("invoiceTypeId")), false);
					...
					try {
						...
						if (UtilValidate.isNotEmpty(invoiceItem.get("productId"))
								&& delegator.findOne("Product",
										UtilMisc.<String, Object> toMap("productId", invoiceItem.get("productId")),
										false) == null) {
	...
	public static String forwardApplication(HttpServletRequest request, HttpServletResponse response) {
		...
		if (documentApproval == null) {
			...
			// Set Responsible
			// responsibleEmployee
			invoice.set("responsibleEmployee", documentApproval.getString("responsibleEmployee"));
