...
-->
 <script type="text/javascript">
 	...
 	function loanApplications(partyId){
    	 var reqUrl = '/loanreports/control/loanApplicationListing';
         getLoanApplicationListing(reqUrl, partyId);
 	}
    ...
    jQuery.ajax({
     ...
     success : function(data){
      ...
      error : function(errorData){
         ...
         function getLoanApplicationListing(reqUrl, partyId){
    jQuery.ajax({
     ...
     data   : {'partyId': partyId}, //here you can pass the parameters to  
                                                   //the request if any.
     success : function(data){
				var options =  jQuery('select[name="loanApplicationId"]');
				options.empty();
				options.append($("<option />").val('').text('Please select Loan Application ..'));
				$.each(data, function(item, itemvalue) {
				    options.append($("<option />").val(item).text(itemvalue));
				});
               },
