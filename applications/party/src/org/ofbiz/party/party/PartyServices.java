	...
	public static Map<String, Object> createPerson(DispatchContext ctx,
		...
		if (UtilValidate.isNotEmpty(partyId) && partyId.matches("\\d+")) {
			return ServiceUtil.returnError(UtilProperties.getMessage(
					resourceError, "party.id_is_digit", locale));
		}
		...
		if (UtilValidate.isEmpty(partyId)) {
			try {
				...
				return ServiceUtil.returnError(UtilProperties.getMessage(
						resourceError, "party.id_generation_failure", locale));
			}
		}
...
//		String partyId_upd = (String) context.get("partyId");
	...
	public static Map<String, Object> setPartyStatus(DispatchContext ctx,
		...
		try {
			...
			if (!party.getString("statusId").equals(statusId)) {
				...
				GenericValue statusValidChange = delegator.findOne(
				...
				if (statusValidChange == null) {
					...
					return ServiceUtil.returnError(UtilProperties.getMessage(
							resource, "PartyStatusCannotBeChanged", UtilMisc
									.toMap("partyFromStatusId",
											party.getString("statusId"),
											"partyToStatusId", statusId),
							locale));
				}
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(
					resourceError, "person.update.write_failure",
					new Object[] { e.getMessage() }, locale));
		}
	}
	...
	public static Map<String, Object> updatePerson(DispatchContext ctx,
		...
		try {
			...
			return ServiceUtil.returnError(UtilProperties.getMessage(
					resourceError, "person.update.read_failure",
					new Object[] { e.getMessage() }, locale));
		}

		if (person == null || party == null) {
			return ServiceUtil.returnError(UtilProperties.getMessage(
					resourceError, "person.update.not_found", locale));
		}
		...
		result.put(ModelService.SUCCESS_MESSAGE, UtilProperties.getMessage(
				resourceError, "person.update.success", locale));
		return result;
	}
		...
		try {
			...
			if (partyGroupPartyType == null) {
				return ServiceUtil
						.returnError(UtilProperties
								.getMessage(
										resourceError,
										"partyservices.partyservices.party_type_not_found_in_database_cannot_create_party_group",
										locale));
			}
			...
			if (party != null) {
				...
				if (!EntityTypeUtil.isType(partyType, partyGroupPartyType)) {
					return ServiceUtil
							.returnError(UtilProperties
									.getMessage(
											resourceError,
											"partyservices.partyservices.cannot_create_party_group_already_exists_not_PARTY_GROUP_type",
											locale));
				}
			} else {
				...
				if (UtilValidate.isNotEmpty(context.get("partyTypeId"))) {
					GenericValue desiredPartyType = delegator.findOne(
							"PartyType",
				...
				Map<String, Object> newPartyMap = UtilMisc.toMap("partyId",
						partyId, "partyTypeId", partyTypeId, "createdDate",
						now, "lastModifiedDate", now);
				if (userLogin != null) {
					newPartyMap.put("createdByUserLogin",
							userLogin.get("userLoginId"));
					newPartyMap.put("lastModifiedByUserLogin",
	...
	public static Map<String, Object> createPartyNote(DispatchContext dctx,
		...
		if (noteId == null) {
			...
			try {
				...
				Debug.logError(e, e.getMessage(), module);
				return ServiceUtil.returnError(UtilProperties.getMessage(
						resource, "PartyNoteCreationError",
		...
		try {
			...
			result.put(ModelService.RESPONSE_MESSAGE,
					ModelService.RESPOND_ERROR);
			result.put(ModelService.ERROR_MESSAGE, UtilProperties.getMessage(
					resourceError,
	...
	public static Map<String, Object> getPartyFromPerson(DispatchContext dctx,
		...
		if (firstName.length() == 0 && lastName.length() == 0) {
			return ServiceUtil.returnError(UtilProperties.getMessage(
					resourceError, "partyservices.both_names_cannot_be_empty",
					locale));
		}
	...
	public static Map<String, Object> createPartyDataSource(
		...
		try {
			...
			if (party == null || dataSource == null) {
				List<String> errorList = UtilMisc.toList(UtilProperties
						.getMessage(resource,
	...
	public static Map<String, Object> findParty(DispatchContext dctx,
		...
		try {
			...
			String errMsg = "Error looking up current PartyType: "
					+ e.toString();
			Debug.logError(e, errMsg, module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource,
					"PartyLookupPartyTypeError",
		...
		if ("Y".equals(lookupFlag)) {
			String showAll = (context.get("showAll") != null ? (String) context
					.get("showAll") : "N");
			paramList = paramList + "&lookupFlag=" + lookupFlag + "&showAll="
					+ showAll + "&extInfo=" + extInfo;
			...
			if (UtilValidate.isNotEmpty(partyRelationshipTypeId)) {
				...
				if (UtilValidate.isEmpty(ownerPartyIds)) {
					...
					relationshipCond = EntityCondition.makeCondition(
							"partyIdFrom", EntityOperator.IN, ownerPartyIds);
				}
				dynamicView.addAlias("PRSHP", "partyIdFrom");
				// add the expr
				andExprs.add(EntityCondition.makeCondition(relationshipCond,
						EntityOperator.AND,
			...
			if (!"Y".equals(showAll)) {
				...
				// now the statusId - send ANY for all statuses; leave null for
				// just enabled; or pass a specific status
				if (statusId != null) {
					...
					// NOTE: _must_ explicitly allow null as it is not included
					// in a not equal in many databases... odd but true
					andExprs.add(EntityCondition.makeCondition(EntityCondition
							.makeCondition("statusId", EntityOperator.EQUALS,
									null), EntityOperator.OR, EntityCondition
				...
				if (roleTypeId != null && !"ANY".equals(roleTypeId)) {
					...
					// add the expr
					andExprs.add(EntityCondition.makeCondition("roleTypeId",
				...
				if (UtilValidate.isNotEmpty(inventoryItemId)) {
					paramList = paramList + "&inventoryItemId="
							+ inventoryItemId;
				...
				if ("P".equals(extInfo)) {
					...
					if (stateProvinceGeoId != null
						...
						paramList = paramList + "&stateProvinceGeoId="
								+ stateProvinceGeoId;
						andExprs.add(EntityCondition.makeCondition(
								"stateProvinceGeoId", EntityOperator.EQUALS,
								stateProvinceGeoId));
					}
				...
				if ("T".equals(extInfo)) {
					...
					if (UtilValidate.isNotEmpty(contactNumber)) {
						paramList = paramList + "&contactNumber="
								+ contactNumber;
						andExprs.add(EntityCondition.makeCondition(
			...
			if (mainCond != null || "Y".equals(showAll)) {
				try {
					...
					EntityFindOptions findOpts = new EntityFindOptions(true,
							...
							EntityFindOptions.CONCUR_READ_ONLY, -1, highIndex,
							true);
					// using list iterator
					EntityListIterator pli = delegator
							.findListIteratorByCondition(dynamicView, mainCond,
	...
	public static Map<String, Object> linkParty(DispatchContext dctx,
		...
		try {
			partyTo = delegator.findOne("Party",
					UtilMisc.toMap("partyId", partyIdTo), false);
		} catch (GenericEntityException e) {
		...
		try {
			delegator.storeByCondition("ProductStoreRole", UtilMisc
					.<String, Object> toMap("partyId", partyIdTo, "thruDate",
							now), EntityCondition.makeCondition("partyId",
					EntityOperator.EQUALS, partyId));
		} catch (GenericEntityException e) {
		...
		try {
			delegator.storeByCondition("CommunicationEventRole", UtilMisc
		...
		for (int i = 0; i < records.length; i++) {
			if (records[i] != null) {
				...
				if (map.length != 2 && map.length != 3) {
					return ServiceUtil.returnError(UtilProperties.getMessage(
							resource, "PartyImportInvalidCsvFile", locale));
				} else {
				...
				return ServiceUtil.returnError(UtilProperties.getMessage(
						resource, "PartyImportNoRecordsFoundInFile", locale));
			}
		}
	 ...
	 *            the dispatch context
	 * @param context
	 *            use to search with partyId or goodIdentification.idValue
	...
	public static Map<String, Object> importParty(DispatchContext dctx,
		...
		ByteBuffer fileBytes = (ByteBuffer) context.get("uploadedFile");
		String encoding = System.getProperty("file.encoding");
		String csvString = Charset.forName(encoding).decode(fileBytes)
				.toString();
		final BufferedReader csvReader = new BufferedReader(new StringReader(
				csvString));
		final CSVFormatBuilder builder = CSVFormat.newBuilder(',')
				.withQuoteChar('"').withHeader();
		...
		try {
			for (final CSVRecord rec : fmt.parse(csvReader)) {
				...
				if (lastPartyId == null || !currentPartyId.equals(lastPartyId)) {
					...
					// party validation
					List<GenericValue> currencyCheck = delegator.findByAnd(
							"Uom", UtilMisc.toMap("abbreviation",
									rec.get("preferredCurrencyUomId"),
									"uomTypeId", "CURRENCY_MEASURE"), null,
							false);
					if (UtilValidate.isNotEmpty(rec
							.get("preferredCurrencyUomId"))
							&& currencyCheck.size() == 0) {
						newErrMsgs.add("Line number " + rec.getRecordNumber()
								+ ": partyId: " + currentPartyId
								+ "Currency code not found for: "
								+ rec.get("preferredCurrencyUomId"));
					}
					...
					if (UtilValidate.isEmpty(rec.get("roleTypeId"))) {
						newErrMsgs
							...
							"RoleType",
							UtilMisc.<String, Object> toMap("roleTypeId",
									rec.get("roleTypeId")), true) == null) {
						newErrMsgs.add("Line number " + rec.getRecordNumber()
								+ ": RoletypeId is not valid: "
								+ rec.get("roleTypeId"));
					}
					...
					if (UtilValidate.isNotEmpty(rec.get("contactMechTypeId"))
							&& delegator.findOne(
									...
									UtilMisc.toMap("contactMechTypeId",
											rec.get("contactMechTypeId"))) == null) {
						newErrMsgs.add("Line number " + rec.getRecordNumber()
								+ ": partyId: " + currentPartyId
					...
					if (UtilValidate.isNotEmpty(rec.get("contactMechTypeId"))
							&& "POSTAL_ADDRESS".equals(rec
									.get("contactMechTypeId"))) {
						if (UtilValidate.isEmpty(rec.get("countryGeoId"))) {
							newErrMsgs.add("Line number "
									+ rec.getRecordNumber() + ": partyId: "
									+ currentPartyId + "Country code missing");
						} else {
							List<GenericValue> countryCheck = delegator
									.findByAnd("Geo", UtilMisc.toMap(
											"geoTypeId", "COUNTRY",
											"abbreviation",
											rec.get("countryGeoId")), null,
											false);
						...
						if (UtilValidate.isEmpty(rec.get("city"))) {
							newErrMsgs.add("Line number "
									+ rec.getRecordNumber() + " partyId: "
									+ currentPartyId + "City name is missing");
						}

						if (UtilValidate.isNotEmpty(rec
								.get("stateProvinceGeoId"))) {
							List<GenericValue> stateCheck = delegator
									.findByAnd("Geo", UtilMisc.toMap(
											"geoTypeId", "STATE",
											"abbreviation",
							...
							if (stateCheck.size() == 0) {
								newErrMsgs.add("Line number "
										+ rec.getRecordNumber() + " partyId: "
										+ currentPartyId
										+ " Invalid stateProvinceGeoId code: "
										+ rec.get("countryGeoId"));
							}
						}
					}
					...
					if (UtilValidate.isNotEmpty(rec.get("contactMechTypeId"))
							&& "EMAIL_ADDRESS".equals(rec
									.get("contactMechTypeId"))) {
						if (UtilValidate.isEmpty(rec.get("emailAddress"))) {
							newErrMsgs
									.add("Line number " + rec.getRecordNumber()
											+ " partyId: " + currentPartyId
					...
					if (errMsgs.size() == 0) {
						List<GenericValue> partyCheck = delegator.findByAnd(
								"PartyIdentification", UtilMisc.toMap(
										"partyIdentificationTypeId",
										"PARTY_IMPORT", "idValue",
										rec.get("partyId")), null, false);
						addParty = partyCheck.size() == 0;
						if (!addParty) { // update party
							newPartyId = EntityUtil.getFirst(partyCheck)
									.getString("partyId");

							if (UtilValidate.isNotEmpty(rec.get("groupName"))) {
								Map<String, Object> partyGroup = UtilMisc
										.toMap("partyId",
												newPartyId,
												"preferredCurrencyUomId",
												rec.get("preferredCurrencyUomId"),
												"groupName", rec
														.get("groupName"),
												"userLogin", userLogin);
								result = dispatcher.runSync("updatePartyGroup",
										partyGroup);
							} else { // person
								Map<String, Object> person = UtilMisc.toMap(
										"partyId", newPartyId, "firstName",
										rec.get("firstName"), "middleName",
										rec.get("midleName"), "lastName",
										rec.get("lastName"),
										"preferredCurrencyUomId",
										rec.get("preferredCurrencyUomId"),
										"userLogin", userLogin);
							...
							if (UtilValidate.isNotEmpty(rec.get("groupName"))) {
								Map<String, Object> partyGroup = UtilMisc
										.toMap("preferredCurrencyUomId", rec
												.get("preferredCurrencyUomId"),
												"groupName", rec
														.get("groupName"),
												"userLogin", userLogin,
												"statusId", "PARTY_ENABLED");
								result = dispatcher.runSync("createPartyGroup",
										partyGroup);
							} else { // person
								Map<String, Object> person = UtilMisc.toMap(
							...
							Map<String, Object> partyIdentification = UtilMisc
									.toMap("partyId", newPartyId,
											"partyIdentificationTypeId",
											"PARTY_IMPORT", "idValue",
											rec.get("partyId"), "userLogin",
											userLogin);
							...
							Map<String, Object> partyRole = UtilMisc.toMap(
									"partyId", newPartyId, "roleTypeId",
									rec.get("roleTypeId"), "userLogin",
									userLogin);
							dispatcher.runSync("createPartyRole", partyRole);

							if (UtilValidate.isNotEmpty(rec
									.get("companyPartyId"))) {
								List<GenericValue> companyCheck = delegator
										.findByAnd(
												"PartyIdentification",
												UtilMisc.toMap(
														"partyIdentificationTypeId",
														...
														rec.get("partyId")),
												null, false);
								if (companyCheck.size() == 0) { // update party
																// group
									// company does not exist so create
									Map<String, Object> companyPartyGroup = UtilMisc
											.toMap("partyId",
													newCompanyPartyId,
													"statusId",
													"PARTY_ENABLED",
									...
									result = dispatcher.runSync(
											"createPartyGroup",
											companyPartyGroup);
									newCompanyPartyId = (String) result
											.get("partyId");
								} else {
									newCompanyPartyId = EntityUtil.getFirst(
											companyCheck).getString("partyId");
								}

								Map<String, Object> companyRole = UtilMisc
										.toMap("partyId", newCompanyPartyId,
												"roleTypeId", "ACCOUNT",
												"userLogin", userLogin);
								...
								Map<String, Object> partyRelationship = UtilMisc
										.toMap("partyIdTo", newPartyId,
												"partyIdFrom",
												newCompanyPartyId,
												"roleTypeIdFrom", "ACCOUNT",
												"partyRelationshipTypeId",
												"EMPLOYMENT", "userLogin",
												userLogin);
								result = dispatcher.runSync(
				...
				currentContactMechPurposeTypeId = rec
						.get("contactMechPurposeTypeId");
				...
				if (newPartyId != null && addParty
					...
					Map<String, Object> emailAddress = UtilMisc.toMap(
							"contactMechTypeId", "EMAIL_ADDRESS", "userLogin",
							userLogin);
					...
					if ("EMAIL_ADDRESS".equals(currentContactMechTypeId)) {
						...
						emailAddressChanged = lastEmailAddress == null
								|| !lastEmailAddress.equals(rec
										.get("emailAddress"));
						lastEmailAddress = rec.get("emailAddress");
					}

					Map<String, Object> postalAddress = UtilMisc.toMap(
							"userLogin", (Object) userLogin); // casting is here
																// necessary for
																// some compiler
																// versions
					...
					if ("POSTAL_ADDRESS".equals(currentContactMechTypeId)) {
						...
						postalAddressChanged = lastAddress1 == null
								|| !lastAddress1.equals(postalAddress
										.get("address1"))
								|| lastAddress2 == null
								|| !lastAddress2.equals(postalAddress
										.get("address2"))
								|| lastCity == null
								|| !lastCity.equals(postalAddress.get("city"))
								|| lastCountryGeoId == null
								|| !lastCountryGeoId.equals(postalAddress
										.get("countryGeoId"));
						...
						lastCountryGeoId = (String) postalAddress
								.get("countryGeoId");
					}

					Map<String, Object> telecomNumber = UtilMisc.toMap(
							"userLogin", (Object) userLogin); // casting is here
																// necessary for
																// some compiler
																// versions
					...
					if ("TELECOM_NUMBER".equals(currentContactMechTypeId)) {
						...
						telecomNumberChanged = lastCountryCode == null
								|| !lastCountryCode.equals(telecomNumber
										.get("countryCode"))
								|| lastAreaCode == null
								|| !lastAreaCode.equals(telecomNumber
										.get("areaCode"))
								|| lastContactNumber == null
								|| !lastContactNumber.equals(telecomNumber
										.get("contactNumber"));
						lastCountryCode = (String) telecomNumber
								.get("countryCode");
						lastAreaCode = (String) telecomNumber.get("areaCode");
						lastContactNumber = (String) telecomNumber
								.get("contactNumber");
					}

					Map<String, Object> partyContactMechPurpose = UtilMisc
							.toMap("partyId", newPartyId, "userLogin",
									userLogin);
					...
					if (currentContactMechPurposeTypeId != null
							&& ("TELECOM_NUMBER"
									.equals(currentContactMechTypeId)
									|| "POSTAL_ADDRESS"
											.equals(currentContactMechTypeId) || "EMAIL_ADDRESS"
										.equals(currentContactMechTypeId))) {
						partyContactMechPurpose.put("contactMechPurposeTypeId",
								currentContactMechPurposeTypeId);
						partyContactMechPurposeChanged = (lastContactMechPurposeTypeId == null || !lastContactMechPurposeTypeId
								.equals(currentContactMechPurposeTypeId))
						...
						Debug.logInfo(
								"===================================last:"
										...
										+ " current: "
										+ currentContactMechPurposeTypeId
										+ " t :" + telecomNumberChanged
										+ " p: " + postalAddressChanged
										+ " e: " + emailAddressChanged
										+ " result: "
		...
		result = ServiceUtil.returnSuccess(partiesCreated
				+ " new parties created");
		return result;
	}

	public static Map<String, Object> createSaccoMember(DispatchContext ctx,
			Map<String, ? extends Object> context) {
		System.out
				.println("########### GOING TO SAVE MEMBER !!!! ############ ");
		...
		// Party ID Might be empty so check it and get next seq party id if
		// empty
		if (UtilValidate.isEmpty(partyId)) {
			try {
				partyId = delegator.getNextSeqId("Party");
		...
		// Check to see if party object exists, if so make sure its if MEMBER
		// type
		GenericValue party = null;
		...
		// if (party != null) {
		// if (!"MEMBER".equals(party.getString("partyTypeId"))) {
		// return ServiceUtil.returnError(UtilProperties.getMessage(
		// resource, "member.create.party_exists_not_member_type",
		// locale));
		// }
		// }
		if (party == null) {
			// Create a party if one does not exist with an initial status from
			// the input
			System.out
					.println("CCCCCCCCCCCCCCCCC CREATING PARTY NOW ##################");
			String statusId = (String) context.get("statusId");
			if (statusId == null) {
				statusId = "PARTY_ENABLED";
			}

			String description = "SACCO MEMBER";
			Map<String, Object> newPartyMap = UtilMisc.toMap("partyId",
					partyId, "partyTypeId", "MEMBER", "description",
					description, "createdDate", now, "lastModifiedDate", now,
					"statusId", statusId);
			String preferredCurrencyUomId = (String) context
					.get("preferredCurrencyUomId");
		...
		try {
			System.out.println("######### TRYING TO SAVE MEMBER ############ ");
			member = delegator.findByPrimaryKey("Member",
					UtilMisc.toMap("partyId", partyId));
		} catch (GenericEntityException e) {
			System.out.println("######### CATCH SAVE MEMBER ############ ");
			Debug.logWarning(e.getMessage(), module);
		}

		// if (member != null) {
		// return ServiceUtil.returnError(UtilProperties.getMessage(resource,
		// "member.create.member_exists", locale));
		// }
		...
		toBeStored.add(member);
		try {
			System.out.println("######### TRYING TO STORE ALL ############ ");
			delegator.storeAll(toBeStored);
		} catch (GenericEntityException e) {
			// e.printStackTrace();
			Debug.logWarning(e.getMessage(), module);
			return ServiceUtil.returnError(UtilProperties.getMessage(resource,
					"member.create.db_error", new Object[] { e.getMessage() },
					locale));
		}
	...
	/**
	 * List Sacco Members Service
	 * 
	 * This is a duplication of findParty service - work in progress to make it
	 * different
	 ***/
	public static Map<String, Object> listSaccoMembers(DispatchContext dctx,
			Map<String, ? extends Object> context) {
		...
		// Set lookup flag to Y String lookupFlag = (String)
		// context.get("lookupFlag");
		// context.put("lookupFlag", new String("Y"));
		...
		String roleTypeId;
		try {
			roleTypeId = "ANY";// (String) context.get("roleTypeId");
			if (UtilValidate.isNotEmpty(roleTypeId)) {
		...
		String partyTypeId;
		try {
			partyTypeId = "MEMBER";// (String) context.get("partyTypeId");
			if (UtilValidate.isNotEmpty(partyTypeId)) {
		...
		// get the lookup flag
		String lookupFlag = "Y";// (String) context.get("lookupFlag");
		...
		try {
			if ("Y".equals(lookupFlag)) {
				...
				if (!"Y".equals(showAll)) {
					...
					// Add Members
					partyTypeId = "MEMBER";// (String)
											// context.get("partyTypeId");

					System.out.println("PPPPPPPPPPPPPPP PPPP ########## ");
					System.out
							.println("PPPPPPPPPPPPPPP The Party Type ID is  ########## "
									+ partyTypeId);
					System.out.println("PPPPPPPPPPPPPPP PPPP ########## ");
					if (UtilValidate.isNotEmpty(partyTypeId)
							&& "MEMBER".equals(partyTypeId)) {
	...
	public static String memberHasNomineeReferee(HttpServletRequest request,
		...
		if ((memberNomineeELI == null) || (memberNomineeELI.size() <= 0)) {
			result.put("nomineesavailabe", "N");
		} else {
			result.put("nomineesavailabe", "Y");
		}
		...
		if ((memberRefereeELI == null) || (memberRefereeELI.size() <= 0)) {
			result.put("refereeavailabe", "N");
		} else {
	...
	/**
	 * @author Japheth Odonya @when Sep 4, 2014 12:21:29 AM Member has Referee
	 * 
	 * */
