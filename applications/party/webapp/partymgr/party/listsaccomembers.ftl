...
<#else>
    <#assign createUrl = "NewMember"/>
</#if>
<div class="button-bar"><a href="<@ofbizUrl>${createUrl}</@ofbizUrl>" class="buttontext create">Create New Member</a></div>
<div class="screenlet">
  <div class="screenlet-title-bar">
    <#if partyList?has_content>
      <ul>
        <#if hideFields == "Y">
          <li class="collapsed"><a href="<@ofbizUrl>listsaccomembers?hideFields=N${paramList}</@ofbizUrl>" title="${uiLabelMap.CommonShowLookupFields}">&nbsp;</a></li>
        <#else>
          <li class="expanded"><a href="<@ofbizUrl>listsaccomembers?hideFields=Y${paramList}</@ofbizUrl>" title="${uiLabelMap.CommonHideFields}">&nbsp;</a></li>
        </#if>
        <#if (partyListSize > 0)>
          <#if (partyListSize > highIndex)>
            <li><a class="nav-next" href="<@ofbizUrl>listsaccomembers?VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex+1}&amp;hideFields=${hideFields}${paramList}</@ofbizUrl>">${uiLabelMap.CommonNext}</a></li>
          <#else>
            <li class="disabled">${uiLabelMap.CommonNext}</li>
          </#if>
          <li>${lowIndex} - ${highIndex} ${uiLabelMap.CommonOf} ${partyListSize}</li>
          <#if (viewIndex > 0)>
            <li><a class="nav-previous" href="<@ofbizUrl>listsaccomembers?VIEW_SIZE=${viewSize}&amp;VIEW_INDEX=${viewIndex-1}&amp;hideFields=${hideFields}${paramList}</@ofbizUrl>">${uiLabelMap.CommonPrevious}</a></li>
          <#else>
