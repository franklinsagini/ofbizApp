...
public class ModelFormField {
	...
	public ModelFormField(Element fieldElement, ModelForm modelForm) {
		...
		this.setEntryName(UtilXml.checkEmpty(
				fieldElement.getAttribute("entry-name"), this.name));
		this.parameterName = UtilXml.checkEmpty(
				fieldElement.getAttribute("parameter-name"), this.name);
		this.fieldName = UtilXml.checkEmpty(
				fieldElement.getAttribute("field-name"), this.name);
		this.attributeName = UtilXml.checkEmpty(
				fieldElement.getAttribute("attribute-name"), this.name);
		this.setTitle(fieldElement.hasAttribute("title") ? fieldElement
		...
		this.requiredFieldStyle = fieldElement
				.getAttribute("required-field-style");
		this.sortFieldStyle = fieldElement.getAttribute("sort-field-style");
		this.sortFieldAscStyle = fieldElement
				.getAttribute("sort-field-asc-style");
		this.sortFieldDescStyle = fieldElement
				.getAttribute("sort-field-desc-style");
		this.redWhen = fieldElement.getAttribute("red-when");
		this.setUseWhen(fieldElement.getAttribute("use-when"));
		this.encodeOutput = !"false".equals(fieldElement
				.getAttribute("encode-output"));
		this.event = fieldElement.getAttribute("event");
		this.setAction(fieldElement.hasAttribute("action") ? fieldElement
				.getAttribute("action") : null);
		this.idName = fieldElement.getAttribute("id-name");
		this.separateColumn = "true".equals(fieldElement
				.getAttribute("separate-column"));
		this.requiredField = fieldElement.hasAttribute("required-field") ? "true"
				.equals(fieldElement.getAttribute("required-field")) : null;
		this.sortField = fieldElement.hasAttribute("sort-field") ? "true"
				.equals(fieldElement.getAttribute("sort-field")) : null;
		this.sortFieldHelpText = fieldElement
				.getAttribute("sort-field-help-text");
		...
		try {
			...
			Debug.logError(e,
					"Could not convert position attribute of the field element to an integer: ["
							+ positionStr
							+ "], using the default of the form renderer",
					module);
		}
		...
		for (Element subElement : subElements) {
			...
			if (UtilValidate.isEmpty(subElementName)) {
				...
				throw new IllegalArgumentException(
						"The field sub-element with name " + subElementName
								+ " is not supported");
		}
	}
	...
	public void addOnEventUpdateArea(UpdateArea updateArea) {
		...
		Debug.logInfo(this.modelForm.getName() + ":" + this.name
				+ " adding UpdateArea type " + updateArea.getEventType(),
				module);
	...
	public void mergeOverrideModelFormField(ModelFormField overrideFormField) {
		...
		if (UtilValidate.isNotEmpty(overrideFormField.mapAcsr))
			this.mapAcsr = overrideFormField.mapAcsr; // Debug.logInfo("overriding mapAcsr, old="
														// +
														// (this.mapAcsr==null?"null":this.mapAcsr.getOriginalName())
														// + ", new=" +
														// overrideFormField.mapAcsr.getOriginalName(),
														// module);
	...
	public boolean induceFieldInfoFromServiceParam(String defaultFieldType) {
		...
		try {
			ModelService modelService = dispatchContext.getModelService(this
					.getServiceName());
			if (modelService != null) {
				ModelParam modelParam = modelService.getParam(this
						.getAttributeName());
				if (modelParam != null) {
			...
			Debug.logError(e,
					"error getting service parameter definition for auto-field with serviceName: "
							+ this.getServiceName() + ", and attributeName: "
	...
	public boolean induceFieldInfoFromServiceParam(ModelService modelService,
		...
		if ("find".equals(defaultFieldType)) {
			if (modelParam.type.indexOf("Double") != -1
				...
				ModelFormField.RangeFindField textField = new ModelFormField.RangeFindField(
						ModelFormField.FieldInfo.SOURCE_AUTO_SERVICE, this);
				textField.setSize(6);
				this.setFieldInfo(textField);
			} else if (modelParam.type.indexOf("Timestamp") != -1) {
				ModelFormField.DateFindField dateTimeField = new ModelFormField.DateFindField(
						ModelFormField.FieldInfo.SOURCE_AUTO_SERVICE, this);
				...
				ModelFormField.TextFindField textField = new ModelFormField.TextFindField(
						ModelFormField.FieldInfo.SOURCE_AUTO_SERVICE, this);
				this.setFieldInfo(textField);
			}
		} else if ("display".equals(defaultFieldType)) {
			ModelFormField.DisplayField displayField = new ModelFormField.DisplayField(
					ModelFormField.FieldInfo.SOURCE_AUTO_SERVICE, this);
			...
			if (modelParam.type.indexOf("Double") != -1
					...
					|| modelParam.type.indexOf("Long") != -1
					|| modelParam.type.indexOf("Integer") != -1) {
				ModelFormField.TextField textField = new ModelFormField.TextField(
						ModelFormField.FieldInfo.SOURCE_AUTO_SERVICE, this);
				textField.setSize(6);
				this.setFieldInfo(textField);
			} else if (modelParam.type.indexOf("Timestamp") != -1) {
				ModelFormField.DateTimeField dateTimeField = new ModelFormField.DateTimeField(
						ModelFormField.FieldInfo.SOURCE_AUTO_SERVICE, this);
				dateTimeField.setType("timestamp");
				this.setFieldInfo(dateTimeField);
			} else if (modelParam.type.indexOf("Date") != -1) {
	...
	public boolean induceFieldInfoFromEntityField(String defaultFieldType) {
		...
		try {
			ModelEntity modelEntity = entityModelReader.getModelEntity(this
					.getEntityName());
			if (modelEntity != null) {
				ModelField modelField = modelEntity.getField(this
						.getFieldName());
				if (modelField != null) {
	...
	public boolean induceFieldInfoFromEntityField(ModelEntity modelEntity,
		...
		if ("find".equals(defaultFieldType)) {
			if ("id".equals(modelField.getType())
				...
				ModelFormField.TextFindField textField = new ModelFormField.TextFindField(
						ModelFormField.FieldInfo.SOURCE_AUTO_ENTITY, this);
			...
			ModelFormField.HiddenField hiddenField = new ModelFormField.HiddenField(
					ModelFormField.FieldInfo.SOURCE_AUTO_SERVICE, this);
			...
			if ("id".equals(modelField.getType())
				...
				ModelFormField.DropDownField dropDownField = new ModelFormField.DropDownField(
						ModelFormField.FieldInfo.SOURCE_AUTO_ENTITY, this);
				dropDownField.setAllowEmpty(false);
				dropDownField.addOptionSource(new ModelFormField.SingleOption(
						"Y", null, dropDownField));
				dropDownField.addOptionSource(new ModelFormField.SingleOption(
						"N", null, dropDownField));
				this.setFieldInfo(dropDownField);
				// ModelFormField.TextField textField = new
				// ModelFormField.TextField(ModelFormField.FieldInfo.SOURCE_AUTO_ENTITY,
				// this);
				// textField.setSize(1);
				...
				ModelFormField.TextareaField textareaField = new ModelFormField.TextareaField(
						ModelFormField.FieldInfo.SOURCE_AUTO_ENTITY, this);
	...
	public void renderFieldString(Appendable writer,
			Map<String, Object> context, FormStringRenderer formStringRenderer)
			throws IOException {
	...
	public void setFieldInfo(FieldInfo fieldInfo) {
		...
		if (this.fieldInfo == null
				|| (fieldInfo.getFieldSource() <= this.fieldInfo
						.getFieldSource())) {
	 ...
	 * with this field. This can be used to get additional information about the
	 * field. Use the getServiceName() method to get the Entity name that the
	 * field is in.
	 * 
	 ...
	 * is true) then the value will be retreived from the parameters Map in the
	 * context.
	 * 
	 * @param context
	 *            the context
	...
	public String getEntry(Map<String, ? extends Object> context,
		...
		Boolean useRequestParameters = (Boolean) context
				.get("useRequestParameters");
		...
		// if useRequestParameters is TRUE then parameters will always be used,
		// if FALSE then parameters will never be used
		// if isError is TRUE and useRequestParameters is not FALSE (ie is null
		// or TRUE) then parameters will be used
		if ((Boolean.TRUE.equals(isError) && !Boolean.FALSE
				.equals(useRequestParameters))
				|| (Boolean.TRUE.equals(useRequestParameters))) {
			// Debug.logInfo("Getting entry, isError true so getting from parameters for field "
			// + this.getName() + " of form " + this.modelForm.getName(),
			// module);
			Map<String, Object> parameters = UtilGenerics.checkMap(
					context.get("parameters"), String.class, Object.class);
			...
			if (parameters != null && parameters.get(parameterName) != null) {
				...
				if (parameterValue instanceof String) {
					...
					// we might want to do something else here in the future,
					// but for now this is probably best
					Debug.logWarning(
							"Found a non-String parameter value for field ["
									+ this.getModelForm().getName() + "."
			...
			// Debug.logInfo("Getting entry, isError false so getting from Map in context for field "
			// + this.getName() + " of form " + this.modelForm.getName(),
			// module);
			Map<String, ? extends Object> dataMap = this.getMap(context);
			boolean dataMapIsContext = false;
			if (dataMap == null) {
				// Debug.logInfo("Getting entry, no Map found with name " +
				// this.getMapName() + ", using context for field " +
				// this.getName() + " of form " + this.modelForm.getName(),
				// module);
				dataMap = context;
			...
			if (UtilValidate.isNotEmpty(this.entryAcsr)) {
				...
				if (dataMap instanceof GenericEntity) {
					...
					if (genEnt.getModelEntity().isField(
							this.entryAcsr.getOriginalName())) {
						retVal = genEnt.get(this.entryAcsr.getOriginalName(),
								locale);
					} else {
						// TODO: this may never come up, but if necessary use
						// the FlexibleStringExander to eval the name first:
						// String evaled = this.entryAcsr
					}
				} else {
					retVal = this.entryAcsr.get(dataMap, locale);
				}
			} else {
				// Debug.logInfo("Getting entry, no entryAcsr so using field name "
				// + this.name + " for field " + this.getName() + " of form " +
				// this.modelForm.getName(), module);
				// if no entry name was specified, use the field's name
				retVal = dataMap.get(this.name);
			}

			// this is a special case to fill in fields during a create by
			// default from parameters passed in
			if (dataMapIsContext && retVal == null
					&& !Boolean.FALSE.equals(useRequestParameters)) {
				Map<String, ? extends Object> parameters = UtilGenerics
						.checkMap(context.get("parameters"));
				if (parameters != null) {
					if (UtilValidate.isNotEmpty(this.entryAcsr))
			...
			if (retVal != null) {
				...
				if (retVal instanceof Double || retVal instanceof Float
					...
					DateFormat df = UtilDateTime.toDateFormat(
							UtilDateTime.DATE_FORMAT, timeZone, null);
					return df.format((java.util.Date) retVal);
				} else if (retVal instanceof java.sql.Time) {
					DateFormat df = UtilDateTime.toTimeFormat(
							UtilDateTime.TIME_FORMAT, timeZone, null);
					return df.format((java.util.Date) retVal);
				} else if (retVal instanceof java.sql.Timestamp) {
					DateFormat df = UtilDateTime.toDateTimeFormat(
							UtilDateTime.DATE_TIME_FORMAT, timeZone, null);
					return df.format((java.util.Date) retVal);
				} else if (retVal instanceof java.util.Date) {
					DateFormat df = UtilDateTime.toDateTimeFormat(
							"EEE MMM dd hh:mm:ss z yyyy", timeZone, null);
					return df.format((java.util.Date) retVal);
				} else {
	...
	public Map<String, ? extends Object> getMap(
		...
		if (UtilValidate.isEmpty(this.mapAcsr))
			return this.modelForm.getDefaultMap(context); // Debug.logInfo("Getting Map from default of the form because of no mapAcsr for field "
															// + this.getName(),
															// module);

		// Debug.logInfo("Getting Map from mapAcsr for field " + this.getName()
		// + ", map-name=" + mapAcsr.getOriginalName() + ", context type=" +
		// context.getClass().toString(), module);
		Map<String, ? extends Object> result = null;
		try {
			result = mapAcsr.get(context);
		} catch (java.lang.ClassCastException e) {
			String errMsg = "Got an unexpected object type (not a Map) for map-name ["
					+ mapAcsr.getOriginalName()
					+ "] in field with name ["
	...
	/**
	 * Gets the name of the Entity Field that corresponds with this field. This
	 * can be used to get additional information about the field. Use the
	 * getEntityName() method to get the Entity name that the field is in.
	 * 
	 ...
	 * pre-populate the field widget when not in an error condition. In an error
	 * condition the parameter name is used to get the value from the parameters
	 * Map.
	 * 
	 * @return returns the name of the Map in the form context that contains the
	 *         entry
	 */
	 ...
	 * interpreter. For HTML forms this is the request parameter name.
	 * 
	 * @return returns the name to use for the parameter for this field in the
	 *         form interpreter
	 */
	 ...
	 * (for ex. thruDate), after-now (for ex. fromDate), or by-name (if the
	...
	public boolean shouldBeRed(Map<String, Object> context) {
		...
		if (UtilValidate.isEmpty(redCondition)
			...
			if ("fromDate".equals(this.name)
					|| (this.entryAcsr != null && "fromDate"
							.equals(this.entryAcsr.getOriginalName()))) {
				redCondition = "after-now";
			} else if ("thruDate".equals(this.name)
					|| (this.entryAcsr != null && "thruDate"
							.equals(this.entryAcsr.getOriginalName()))) {
	...
	public String getTitle(Map<String, Object> context) {
		...
		Map<String, String> uiLabelMap = UtilGenerics.checkMap(context
				.get("uiLabelMap"));
		if (uiLabelMap != null) {
			...
			Debug.logWarning(
					"Could not find uiLabelMap in context while rendering form "
							+ this.modelForm.getName(), module);
		}
	...
	public boolean shouldUse(Map<String, Object> context) {
		...
		try {
				...
				throw new IllegalArgumentException(
						"Return value from use-when condition eval was not a Boolean: "
								+ (retVal != null ? retVal.getClass().getName()
										: "null") + " [" + retVal
								+ "] on the field " + this.name + " of form "
			...
			String errMsg = "Error evaluating BeanShell use-when condition ["
					+ useWhenStr + "] on the field " + this.name + " of form "
	...
	public static abstract class FieldInfo {
		...
		/** Value Constructor */
		public FieldInfo(int fieldSource, int fieldType,
		...
		public static int findFieldTypeFromName(String name) {
			...
			if (fieldTypeInt != null) {
				...
				throw new IllegalArgumentException(
						"Could not get fieldType for field type name " + name);
			}
		}

		public abstract void renderFieldString(Appendable writer,
	...
	public static abstract class FieldInfoWithOptions extends FieldInfo {
		...
		public FieldInfoWithOptions(Element element,
			...
			if (childElements.size() > 0) {
				for (Element childElement : childElements) {
					...
					} else if ("entity-options".equals(childElement
							.getTagName())) {
				...
				// this must be added or the multi-form select box options would
				// not show up
		...
		public void setNoCurrentSelectedKey(String string) {
			this.noCurrentSelectedKey = FlexibleStringExpander
					.getInstance(string);
		}
	...
	public static class SingleOption extends OptionSource {
		...
		public SingleOption(String key, String description, FieldInfo fieldInfo) {
			...
			this.description = FlexibleStringExpander.getInstance(UtilXml
					.checkEmpty(description, key));
			this.fieldInfo = fieldInfo;
		}

		public SingleOption(Element optionElement, FieldInfo fieldInfo) {
			this.key = FlexibleStringExpander.getInstance(optionElement
					.getAttribute("key"));
			this.description = FlexibleStringExpander.getInstance(UtilXml
					.checkEmpty(optionElement.getAttribute("description"),
	...
	public static class ListOptions extends OptionSource {
		...
		public ListOptions(String listName, String listEntryName,
				String keyName, String description, FieldInfo fieldInfo) {
		...
		public ListOptions(Element optionElement, FieldInfo fieldInfo) {
			...
			this.keyAcsr = FlexibleMapAccessor.getInstance(optionElement
					.getAttribute("key-name"));
			this.listAcsr = FlexibleMapAccessor.getInstance(optionElement
					.getAttribute("list-name"));
			this.listEntryName = optionElement.getAttribute("list-entry-name");
			this.description = FlexibleStringExpander.getInstance(optionElement
		...
		public void addOptionValues(List<OptionValue> optionValues,
				Map<String, Object> context, Delegator delegator) {
			List<? extends Object> dataList = UtilGenerics
					.checkList(this.listAcsr.get(context));
			if (dataList != null && dataList.size() != 0) {
				for (Object data : dataList) {
					...
					if (UtilValidate.isNotEmpty(this.listEntryName)) {
						...
						Map<String, Object> dataMap = UtilGenerics
								.checkMap(data);
					...
					if (keyObj instanceof String) {
						...
						try {
							key = (String) ObjectType.simpleTypeConvert(keyObj,
									"String", null, null);
						} catch (GeneralException e) {
							String errMsg = "Could not convert field value for the field: ["
									+ this.keyAcsr.toString()
									+ "] to String for the value ["
									+ keyObj
									+ "]: " + e.toString();
							Debug.logError(e, errMsg, module);
						}
					}
					optionValues.add(new OptionValue(key, description
							.expandString(localContext)));
				}
			}
		}
	}
	...
	public static class EntityOptions extends OptionSource {
		...
		public EntityOptions(Element entityOptionsElement, FieldInfo fieldInfo) {
			...
			this.keyFieldName = entityOptionsElement
					.getAttribute("key-field-name");
			this.description = FlexibleStringExpander
					.getInstance(entityOptionsElement
							.getAttribute("description"));
			this.cache = !"false".equals(entityOptionsElement
					.getAttribute("cache"));
			this.filterByDate = entityOptionsElement
					.getAttribute("filter-by-date");

			List<? extends Element> constraintElements = UtilXml
					.childElementList(entityOptionsElement, "entity-constraint");
			if (UtilValidate.isNotEmpty(constraintElements)) {
				...
				for (Element constraintElement : constraintElements) {
					constraintList.add(new EntityFinderUtil.ConditionExpr(
							constraintElement));
				}
			}

			List<? extends Element> orderByElements = UtilXml.childElementList(
					entityOptionsElement, "entity-order-by");
		...
		public String getKeyFieldName() {
			...
			return this.fieldInfo.getModelFormField().getFieldName(); // get the
																		// modelFormField
																		// fieldName
		}
		...
		public void addOptionValues(List<OptionValue> optionValues,
			...
			if (UtilValidate.isNotEmpty(this.constraintList)) {
				...
				for (EntityFinderUtil.Condition condition : constraintList) {
					ModelEntity modelEntity = delegator
							.getModelEntity(this.entityName);
					if (modelEntity == null) {
						throw new IllegalArgumentException(
								"Error in entity-options: could not find entity ["
										+ this.entityName + "]");
					}
					EntityCondition createdCondition = condition
							.createCondition(context, modelEntity, delegator
									.getModelFieldTypeReader(modelEntity));
					if (createdCondition != null) {
						expandedConditionList.add(createdCondition);
					}
				}
				findCondition = EntityCondition
						.makeCondition(expandedConditionList);
			}
			...
			try {
				...
				values = delegator.findList(this.entityName, findCondition,
						null, this.orderByList, null, this.cache);
				...
				for (GenericValue value : values) {
					...
					// expanding ${} stuff, passing locale explicitly to expand
					// value string because it won't be found in the Entity
					MapStack<String> localContext = MapStack.create(context);
					// Rendering code might try to modify the GenericEntity
					// instance,
					// so we make a copy of it.
					Map<String, Object> genericEntityClone = UtilGenerics
							.cast(value.clone());
					localContext.push(genericEntityClone);

					// expand with the new localContext, which is locale aware
					String optionDesc = this.description.expandString(
							localContext, locale);
					...
					if (keyFieldObject == null) {
						throw new IllegalArgumentException(
								"The entity-options identifier (from key-name attribute, or default to the field name) ["
										...
										+ "], may not be a valid key field name for the entity ["
										+ this.entityName + "].");
					}
					String keyFieldValue = keyFieldObject.toString();
					optionValues
							.add(new OptionValue(keyFieldValue, optionDesc));
				}
			} catch (GenericEntityException e) {
	...
	public static class InPlaceEditor {
		...
		public InPlaceEditor(Element element) {
			...
			this.fieldPostCreation = element
					.getAttribute("field-post-creation");
			this.formClassName = element.getAttribute("form-class-name");
			this.highlightColor = element.getAttribute("highlight-color");
			this.highlightEndColor = element
					.getAttribute("highlight-end-color");
			...
			this.textBeforeControls = element
					.getAttribute("text-before-controls");
			this.textAfterControls = element
					.getAttribute("text-after-controls");
			this.textBetweenControls = element
					.getAttribute("text-between-controls");
			this.updateAfterRequestCall = element
					.getAttribute("update-after-request-call");
		...
		public void setFieldMap(
				Map<FlexibleMapAccessor<Object>, Object> fieldMap) {
	...
	public static class DisplayField extends FieldInfo {
		...
		protected FlexibleStringExpander description;
		protected String type; // matches type of field, currently text or
								// currency
		...
		public DisplayField(Element element, ModelFormField modelFormField) {
			...
			this.alsoHidden = !"false".equals(element
					.getAttribute("also-hidden"));
			this.setDefaultValue(element.getAttribute("default-value"));

			Element inPlaceEditorElement = UtilXml.firstChildElement(element,
					"in-place-editor");
		...
		public String getDescription(Map<String, Object> context) {
			...
			if (UtilValidate.isEmpty(retVal)) {
				...
				retVal = retVal.replaceAll("&nbsp;", " "); // FIXME : encoding
															// currency is a
															// problem for some
															// locale, we should
															// not have any
				...
				try {
					BigDecimal parsedRetVal = (BigDecimal) ObjectType
							.simpleTypeConvert(retVal, "BigDecimal", null,
									null, locale, true);
					retVal = UtilFormatOut.formatCurrency(parsedRetVal,
							isoCode, locale, 10); // we set the max to 10 digits
													// as an hack to not round
													// numbers in the ui
				} catch (GeneralException e) {
				...
				try {
					...
					String errMsg = "Error formatting date using default instead ["
							+ retVal + "]: " + e.toString();
					Debug.logError(e, errMsg, module);
				...
				try {
					...
					DateFormat dateFormatter = UtilDateTime.toDateTimeFormat(
							null, timeZone, locale);
					retVal = dateFormatter.format(date);
				} catch (ConversionException e) {
					String errMsg = "Error formatting date/time using default instead ["
							+ retVal + "]: " + e.toString();
					Debug.logError(e, errMsg, module);
				...
				try {
					Double parsedRetVal = (Double) ObjectType
							.simpleTypeConvert(retVal, "Double", null, locale,
									false);
					String template = UtilProperties.getPropertyValue(
							"arithmetic", "accounting-number.format",
							"#,##0.00;(#,##0.00)");
					retVal = UtilFormatOut.formatDecimalNumber(
							parsedRetVal.doubleValue(), template, locale);
				} catch (GeneralException e) {
			...
			if (UtilValidate.isNotEmpty(this.description) && retVal != null
		...
		/**
		 * @param b
		 *            the field is also hidden true/false
		 */
		...
		/**
		 * @param value
		 *            the value of the image location
		 */
	...
	public static class DisplayEntityField extends DisplayField {
		...
		public String getDescription(Map<String, Object> context) {
			...
			// rather than using the context to expand the string, lookup the
			// given entity and use it to expand the string
			...
			if (!this.entityName.equals("MemberStatus")
					...
					&& !this.entityName.equals("MemberType")
			...
			// try to get the entry for the field if description doesn't expand
			// to anything
			if (UtilValidate.isEmpty(retVal))
				retVal = fieldValue;
			if (UtilValidate.isEmpty(retVal))
	...
	public static class HyperlinkField extends FieldInfo {
		...
		protected List<WidgetWorker.Parameter> parameterList = FastList
				.newInstance();
		...
		public HyperlinkField(Element element, ModelFormField modelFormField) {
			...
			this.targetWindowExdr = FlexibleStringExpander.getInstance(element
					.getAttribute("target-window"));
			this.parametersMapAcsr = FlexibleMapAccessor.getInstance(element
					.getAttribute("parameters-map"));
			this.size = element.getAttribute("size");
			this.setRequestConfirmation("true".equals(element
					.getAttribute("request-confirmation")));
			this.setConfirmationMsg(element
					.getAttribute("confirmation-message"));
			List<? extends Element> parameterElementList = UtilXml
					.childElementList(element, "parameter");
			for (Element parameterElement : parameterElementList) {
				this.parameterList.add(new WidgetWorker.Parameter(
						parameterElement));
			}
			Element autoServiceParamsElement = UtilXml.firstChildElement(
					element, "auto-parameters-service");
			if (autoServiceParamsElement != null) {
				autoServiceParameters = new WidgetWorker.AutoServiceParameters(
						autoServiceParamsElement);
			}
			Element autoEntityParamsElement = UtilXml.firstChildElement(
					element, "auto-parameters-entity");
			if (autoEntityParamsElement != null) {
				autoEntityParameters = new WidgetWorker.AutoEntityParameters(
						autoEntityParamsElement);
			}
		}
		...
		public String getConfirmation(Map<String, Object> context) {
			...
			if (getRequestConfirmation()) {
				String defaultMessage = UtilProperties.getPropertyValue(
						"general", "default.confirmation.message",
		...
		public Map<String, String> getParameterMap(Map<String, Object> context) {
			...
			Map<String, String> addlParamMap = this.parametersMapAcsr
					.get(context);
			...
			if (autoServiceParameters != null) {
				fullParameterMap.putAll(autoServiceParameters.getParametersMap(
						context, this.getModelFormField().getModelForm()
								.getDefaultServiceName()));
			}

			if (autoEntityParameters != null) {
				fullParameterMap.putAll(autoEntityParameters.getParametersMap(
						context, this.getModelFormField().getModelForm()
	...
	public static class SubHyperlink {
		...
		public SubHyperlink(Element element, ModelFormField modelFormField) {
			...
			setRequestConfirmation("true".equals(element
					.getAttribute("request-confirmation")));
		...
		public Map<String, String> getParameterMap(Map<String, Object> context) {
			 ...
			 * hyperlink element: Map<String, String> addlParamMap =
			 * this.parametersMapAcsr.get(context); if (addlParamMap != null) {
			 * fullParameterMap.putAll(addlParamMap); }
			 */
		...
		public boolean shouldUse(Map<String, Object> context) {
			...
			if (UtilValidate.isNotEmpty(useWhen)) {
				try {
					Interpreter bsh = (Interpreter) context
							.get("bshInterpreter");
					if (bsh == null) {
					...
					Object retVal = bsh.eval(StringUtil
							.convertOperatorSubstitutions(useWhen));
	...
	public static class TextField extends FieldInfo {
		...
		protected FlexibleStringExpander placeholder = FlexibleStringExpander
				.getInstance(null);
		...
		public TextField(Element element, ModelFormField modelFormField) {
			...
			try {
				...
				if (UtilValidate.isNotEmpty(sizeStr)) {
					Debug.logError(
							"Could not parse the size value of the text element: ["
									+ sizeStr + "], setting to the default of "
									+ size, module);
				}
			}
			...
			try {
				...
				if (UtilValidate.isNotEmpty(maxlengthStr)) {
					Debug.logError(
							"Could not parse the max-length value of the text element: ["
									+ maxlengthStr
									+ "], setting to null; default of no maxlength will be used",
							module);
				}
			}
			...
			this.clientAutocompleteField = !"false".equals(element
					.getAttribute("client-autocomplete-field"));
	...
	public static class TextareaField extends FieldInfo {
		...
		public TextareaField(Element element, ModelFormField modelFormField) {
			...
			visualEditorEnable = "true".equals(element
					.getAttribute("visual-editor-enable"));
			visualEditorButtons = FlexibleStringExpander.getInstance(element
					.getAttribute("visual-editor-buttons"));
		...
		/**
		 * @param visualEditorEnable
		 *            is visualEditor enabled
		 */
	...
	public static class DateTimeField extends FieldInfo {
		...
		public DateTimeField(ModelFormField modelFormField) {
			super(FieldInfo.SOURCE_EXPLICIT, FieldInfo.DATE_TIME,
					modelFormField);
		}
		...
		/**
		 * Returns the default-value if specified, otherwise the current date,
		 * time or timestamp
		 * 
		 * @param context
		 *            Context Map
	...
	public static class DropDownField extends FieldInfoWithOptions {
		...
		public DropDownField(ModelFormField modelFormField) {
			super(FieldInfo.SOURCE_EXPLICIT, FieldInfo.DROP_DOWN,
					modelFormField);
		}
		...
		public DropDownField(Element element, ModelFormField modelFormField) {
			...
			this.allowEmpty = "true"
					.equals(element.getAttribute("allow-empty"));
			this.allowMulti = "true".equals(element
					.getAttribute("allow-multiple"));
			this.currentDescription = FlexibleStringExpander
					.getInstance(element.getAttribute("current-description"));
		 ...
		 * @param context
		 *            the context
		 * @return returns the name to use for the parameter for this field in
		...
		public String getParameterNameOther(Map<String, Object> context) {
			...
			Integer itemIndex = (Integer) context.get("itemIndex");
			if (itemIndex != null
					&& "multi".equals(this.modelFormField.modelForm.getType())) {
				return baseName
						+ this.modelFormField.modelForm.getItemIndexSeparator()
						+ itemIndex.intValue();
			} else {
				return baseName;
			}
	...
	public static class CheckField extends FieldInfoWithOptions {
		...
		public CheckField(Element element, ModelFormField modelFormField) {
			super(element, modelFormField);
			allChecked = FlexibleStringExpander.getInstance(element
	...
	public static class SubmitField extends FieldInfo {
		...
		public SubmitField(Element element, ModelFormField modelFormField) {
			...
			this.backgroundSubmitRefreshTargetExdr = FlexibleStringExpander
					.getInstance(element
		...
		public String getBackgroundSubmitRefreshTarget(
				Map<String, Object> context) {
	...
	public static class TextFindField extends TextField {
		...
		public TextFindField(Element element, ModelFormField modelFormField) {
			...
			this.ignoreCase = "true"
					.equals(element.getAttribute("ignore-case"));
			this.hideIgnoreCase = "true".equals(element
					.getAttribute("hide-options"))
					|| "ignore-case".equals(element
							.getAttribute("hide-options")) ? true : false;
			...
			this.hideOptions = "true".equals(element
					.getAttribute("hide-options"))
	...
	public static class DateFindField extends DateTimeField {
		...
		public DateFindField(Element element, ModelFormField modelFormField) {
			super(element, modelFormField);
			this.defaultOptionFrom = element
					.getAttribute("default-option-from");
			this.defaultOptionThru = element
					.getAttribute("default-option-thru");
		}
	...
	public static class LookupField extends TextField {
		...
		public LookupField(Element element, ModelFormField modelFormField) {
			super(element, modelFormField);
			this.formName = FlexibleStringExpander.getInstance(element
					.getAttribute("target-form-name"));
			this.descriptionFieldName = element
			...
			this.initiallyCollapsed = element
		...
		public Boolean getShowDescription() {
			return UtilValidate.isEmpty(this.showDescription) ? null : "true"
					.equals(this.showDescription);
		}
